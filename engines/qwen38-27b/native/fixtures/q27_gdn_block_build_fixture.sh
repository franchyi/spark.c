...
docker run --rm --gpus all --network host --user "$user_id:$group_id" \
  ...
  bash -euo pipefail -c '
    ...
    nvcc -O3 -std=c++20 -arch=sm_121a -lineinfo \
      ...
      -I"$tvm_root/include" \
      engines/qwen38-27b/native/csrc/q27_fp8_decode.cu \
      engines/qwen38-27b/native/csrc/q27_bf16_ab.cu \
      engines/qwen38-27b/native/csrc/q27_gdn.cu \
      engines/qwen38-27b/native/csrc/q27_gdn_flashinfer.cc \
      engines/qwen38-27b/native/csrc/q27_gdn_block.cc \
      engines/qwen38-27b/native/fixtures/q27_gdn_block_fixture_test.cu \
