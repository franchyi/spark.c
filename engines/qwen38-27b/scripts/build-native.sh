...
docker run --rm --network host --user "$user_id:$group_id" \
  ...
  "$rust_image" \
  cargo build --locked --release -p sparkserve-q27 --bin q27-inspect --bin q27-map-inspect --bin q27-pack-scales --bin q27-inspect-scales --bin q27-eager --bin q27-serve --bin q27-consume-equivalence
...
cp "$target_host/release/q27-consume-equivalence" "$repo_root/build/bin/q27-consume-equivalence"
