...
int main(int argc, char** argv) {
  ...
  SparkServeQsaDecodePlan plan = {
      ...
      SPARKSERVE_BACKEND_FLASHINFER_XQA_DECODE,
      1,
      0,
  };
  ...
  SparkServeStatus status = sparkserve_qsa_decode_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = sparkserve_qsa_decode_launch(&caps, &args);
    assert(status.code == SPARKSERVE_STATUS_OK);
  }
