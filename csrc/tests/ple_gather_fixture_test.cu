...
int main(int argc, char** argv) {
  ...
  constexpr size_t kRegionBytes =
      kSlabBytes + kRows * sizeof(SparkServePleRowFragment);
  ...
  SparkServeCoherentRegionConfig region_config = {};
  region_config.struct_size = sizeof(region_config);
  region_config.abi_version = SPARKSERVE_FABRIC_ABI_VERSION;
  region_config.kind = SPARKSERVE_COHERENT_REGION_SLAB;
  region_config.flags = SPARKSERVE_COHERENT_REGION_PREFAULT;
  region_config.payload_bytes = kRegionBytes;
  region_config.required_alignment = 4096;
  SparkServeCoherentRegion* region = nullptr;
  Require(sparkserve_coherent_region_create(&region_config, &region));
  SparkServeCoherentRegionView region_view = {};
  region_view.struct_size = sizeof(region_view);
  region_view.abi_version = SPARKSERVE_FABRIC_ABI_VERSION;
  Require(sparkserve_coherent_region_view(region, &region_view));
  auto* slab = static_cast<uint8_t*>(region_view.host_pointer);
  std::memset(slab, 0xa5, kRegionBytes);
  auto* fragments = reinterpret_cast<SparkServePleRowFragment*>(
  ...
  SparkServePleGatherPlan plan = {
      ...
      SPARKSERVE_BACKEND_SGLANG_PLE_GATHER,
      0,
  };
  SparkServeKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  SparkServeStatus status = sparkserve_ple_gather_query(&caps, &plan, &info);
  assert(status.code == SPARKSERVE_STATUS_OK);
  assert(info.available == 1);

  SparkServePleGatherArgs args = {};
  args.struct_size = sizeof(args);
  args.abi_version = SPARKSERVE_KERNEL_ABI_VERSION;
  args.plan = plan;
  args.coherent_base = region_view.device_pointer;
  args.fragments = reinterpret_cast<const SparkServePleRowFragment*>(
  ...
  status = sparkserve_ple_gather_launch(&caps, &args);
  if (status.code != SPARKSERVE_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == SPARKSERVE_STATUS_OK);
