...
int main(int argc, char** argv) {
  ...
  SparkServeQsaScorePlan plan = {
      ...
      SPARKSERVE_BACKEND_TILELANG_QSA_SCORE,
  };
  SparkServeKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(sparkserve_qsa_score_query(&caps, &plan, &info).code ==
         SPARKSERVE_STATUS_OK);
  assert(info.available == 1);

  SparkServeQsaScoreArgs args = {};
  ...
  SparkServeStatus status = sparkserve_qsa_score_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = sparkserve_qsa_score_launch(&caps, &args);
    assert(status.code == SPARKSERVE_STATUS_OK);
  }
