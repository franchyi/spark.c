...
constexpr uint32_t kHidden = SPARKSERVE_QWEN_EXPERT_HIDDEN;
constexpr uint32_t kIntermediate = SPARKSERVE_QWEN_EXPERT_INTERMEDIATE;
...
extern "C" SparkServeStatus sparkserve_qwen_expert_pack_validate(
  ...
  if (args->fills == 0 || args->fills > SPARKSERVE_QWEN_EXPERT_PACK_MAX_FILLS ||
      args->capacity != SPARKSERVE_QWEN_EXPERT_CAPACITY) {
...
extern "C" SparkServeStatus sparkserve_qwen_expert_pack_launch(
    const SparkServeQwenExpertPackArgs* args) {
  SparkServeStatus status = sparkserve_qwen_expert_pack_validate(args);
