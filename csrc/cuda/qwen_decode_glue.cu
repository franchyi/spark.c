...
SparkServeStatus ValidateGlue(const SparkServeQwenDecodeGlueArgs* args) {
  if (args == nullptr || args->struct_size != sizeof(*args) ||
      args->abi_version != SPARKSERVE_QWEN_DECODE_GLUE_ABI_VERSION ||
...
extern "C" SparkServeStatus sparkserve_qwen_repeat_embedding_launch(
    const SparkServeQwenDecodeGlueArgs* args) {
  SparkServeStatus valid = ValidateGlue(args);
  if (valid.code != SPARKSERVE_STATUS_OK) return valid;
...
extern "C" SparkServeStatus sparkserve_qwen_add_hyper_launch(
    const SparkServeQwenDecodeGlueArgs* args) {
  SparkServeStatus valid = ValidateGlue(args);
  if (valid.code != SPARKSERVE_STATUS_OK) return valid;
...
extern "C" SparkServeStatus sparkserve_qwen_qsa_single_value_launch(
    const SparkServeQwenDecodeGlueArgs* args) {
  SparkServeStatus valid = ValidateGlue(args);
  if (valid.code != SPARKSERVE_STATUS_OK) return valid;
...
extern "C" SparkServeStatus sparkserve_qwen_lm_head_launch(
