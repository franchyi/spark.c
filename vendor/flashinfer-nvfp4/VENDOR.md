...
Spark.C compiles FlashInfer's SM120/121 CUTLASS FP4 GEMM template behind a
...
Run `vendor/tools/fetch-flashinfer.sh` to materialize the ignored source tree at
`vendor/_deps/flashinfer` and verify it before compilation.

## Reuse boundary

`models/qwen3.8-flash-next/native/kernels/cuda/nvfp4_dense_flashinfer.cu` invokes FlashInfer's
...
and launch implementation. Spark.C supplies argument validation, the ABI,
...
`models/qwen3.8-flash-next/native/kernels/cuda/nvfp4_grouped_flashinfer.cu` likewise invokes the upstream
...
the grouped CUTLASS collective and its argument-preparation kernel. Spark.C
...
`models/qwen3.8-flash-next/native/kernels/cuda/nvfp4_silu_cute.cc` uses a different reuse mode because GB10 cannot
...
SM121 kernel binary. At serving time Spark.C links that object, the 34 KiB
...
`models/qwen3.8-flash-next/native/kernels/cuda/nvfp4_quantize_cute.cc` applies the same export technique to
...
`models/qwen3.8-flash-next/native/kernels/cuda/moe_route_flashinfer.cu` is a framework-free adaptation of
...
FlashInfer result. Spark.C's independently built ABI path matches all 640 BF16
outputs bit-for-bit for `M=1,N=640,K=2560`.

The fixture payload is intentionally not committed because it contains model
weights. `models/qwen3.8-flash-next/native/tools/capture-nvfp4-fixture.py` regenerates it from the locked local
...
scales. `models/qwen3.8-flash-next/native/tools/capture-grouped-nvfp4-fixture.py` records the upstream grouped
...
`models/qwen3.8-flash-next/native/tools/capture-silu-nvfp4-fixture.py` records FlashInfer's CuTe result and
...
up, and down weights for experts 0 and 1. `models/qwen3.8-flash-next/native/tools/capture-qwen-moe-fixture.py`
