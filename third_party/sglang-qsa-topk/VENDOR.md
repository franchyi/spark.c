...
SparkServe adapts the Qwen QSA `fast_topk` CUDA implementation from SGLang at
...
The implementation in `csrc/cuda/qsa_topk_sglang.cu` retains SGLang's
...
Torch allocation, and Python. SparkServe supplies validated raw device pointers,
strides, ragged ranges, output storage, and a CUDA stream through its C ABI.

Private fixtures are captured by `scripts/capture-qsa-topk-fixture.py` through
