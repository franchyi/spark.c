...
- Q2 service build: `scripts/build-glm53-q2.sh`
- Optional static adapter: `make glm53-ds4-static CUDA_ARCH=sm_121`

The first complete GLM release builds the pinned model-specific `ds4-server`
directly in the isolated `third_party/_deps/ds4-glm53-q2` checkout. It depends
on no installed ds4 shared library, Python module, SGLang runtime, or llama.cpp
runtime. The optional static SparkServe adapter remains in
`csrc/ds4_glm53_adapter.c` behind `csrc/include/sparkserve/ds4_glm53_api.h`, but
it is not the Q2 service acceptance path.
