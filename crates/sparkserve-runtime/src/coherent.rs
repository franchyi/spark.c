...
use crate::ffi::{
    ...
    sparkserve_coherent_region_create, sparkserve_coherent_region_destroy,
...
impl CoherentRegionOwner {
    pub fn create(config: &CoherentRegionConfig) -> Result<Self, CoherentRegionError> {
        ...
        status_result(unsafe { sparkserve_coherent_region_create(config, &mut raw) })?;
        ...
        if let Err(error) =
            status_result(unsafe { sparkserve_coherent_region_view(handle.as_ptr(), &mut view) })
        {
            // SAFETY: creation transferred unique ownership to this function.
            let _ = unsafe { sparkserve_coherent_region_destroy(handle.as_ptr()) };
            return Err(error);
        }
    ...
    fn destroy(&mut self) -> Result<(), CoherentRegionError> {
        ...
        status_result(unsafe { sparkserve_coherent_region_destroy(handle.as_ptr()) })
    }
}
