...
use sparkserve_runtime::openai_server::{
...
impl QwenThreadBackend {
    fn start(model_root: PathBuf, model_id: String) -> Result<Self, GenerationError> {
        ...
        thread::Builder::new()
            .name("sparkserve-qwen-gpu".to_owned())
            .spawn(move || {
                let mut engine = match qwen_native::QwenNativeEngine::create(&model_root) {
                    Ok(engine) => {
...
fn run_generation(engine: &mut qwen_native::QwenNativeEngine, command: EngineCommand) {
    let EngineCommand { request, events } = command;
    let trace_layers = std::env::var_os("SPARKSERVE_QWEN_TRACE_LAYERS").is_some();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("SparkServe {model_id} ready on http://{bind}");
