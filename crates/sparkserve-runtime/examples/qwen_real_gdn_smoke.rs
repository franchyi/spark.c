...
use sparkserve_runtime::ffi::{
    ...
    sparkserve_gdn_block_finish_launch, sparkserve_gdn_block_prepare_launch,
    sparkserve_gdn_decode_launch, sparkserve_mhc_combine_launch,
    sparkserve_mhc_mix_launch, sparkserve_qwen_bf16_to_f32_launch,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    native("Qwen mHC mix", unsafe {
        sparkserve_mhc_mix_launch(&caps, &mhc)
    })?;
    native("Qwen GDN projections and convolution", unsafe {
        sparkserve_gdn_block_prepare_launch(&caps, &block)
    })?;
    native("Qwen GDN recurrence", unsafe {
        sparkserve_gdn_decode_launch(&caps, &recurrence)
    })?;
    native("Qwen GDN gate and output", unsafe {
        sparkserve_gdn_block_finish_launch(&caps, &block)
    })?;
    native("Qwen mHC combine", unsafe {
        sparkserve_mhc_combine_launch(&caps, &mhc)
    })?;
    stream.synchronize()?;
...
fn convert_bf16(
    ...
    native(stage, unsafe { sparkserve_qwen_bf16_to_f32_launch(&args) })
}
