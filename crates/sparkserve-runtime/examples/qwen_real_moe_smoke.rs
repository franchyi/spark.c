...
use sparkserve_runtime::ffi::{
    ...
    SharedExpertPlan, Status, sparkserve_grouped_nvfp4_launch,
    sparkserve_moe_gate_launch, sparkserve_moe_join_launch,
    sparkserve_moe_route_dispatch, sparkserve_moe_route_finalize,
    sparkserve_segmented_nvfp4_quantize_launch,
    sparkserve_segmented_silu_nvfp4_launch, sparkserve_shared_expert_launch,
};
use sparkserve_runtime::kernel::{
    GroupedNvfp4Spec, KERNEL_ABI_VERSION, SegmentedNvfp4QuantizeSpec,
    SegmentedSiluNvfp4Spec,
};
use sparkserve_runtime::qwen_expert_cache::QwenExpertHotCache;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    native("Qwen real router", unsafe { sparkserve_moe_gate_launch(&caps, &gate) })?;
    stream.synchronize()?;
    ...
    native("Qwen route dispatch", unsafe {
        sparkserve_moe_route_dispatch(&caps, &route_args)
    })?;
    ...
    native("Qwen routed input quantize", unsafe {
        sparkserve_segmented_nvfp4_quantize_launch(&caps, &quantize)
    })?;
    ...
    native("Qwen grouped gate/up", unsafe {
        sparkserve_grouped_nvfp4_launch(&caps, &w13)
    })?;
    ...
    native("Qwen fused SiLU quantize", unsafe {
        sparkserve_segmented_silu_nvfp4_launch(&caps, &silu)
    })?;
    ...
    native("Qwen grouped down", unsafe {
        sparkserve_grouped_nvfp4_launch(&caps, &w2)
    })?;
    native("Qwen route finalize", unsafe {
        sparkserve_moe_route_finalize(&caps, &route_args)
    })?;
    ...
    native("Qwen shared expert", unsafe {
        sparkserve_shared_expert_launch(&caps, &shared)
    })?;
    ...
    native("Qwen routed/shared join", unsafe {
        sparkserve_moe_join_launch(&caps, &join)
    })?;
    stream.synchronize()?;
