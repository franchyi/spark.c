...
use sparkserve_runtime::ffi::{
    ...
    sparkserve_qwen_runtime_gdn_finish as sparkserve_gdn_block_finish_launch,
    sparkserve_qwen_runtime_gdn_prepare as sparkserve_gdn_block_prepare_launch,
    sparkserve_qwen_runtime_gdn_decode as sparkserve_gdn_decode_launch,
    sparkserve_qwen_runtime_grouped_nvfp4 as sparkserve_grouped_nvfp4_launch,
    sparkserve_qwen_runtime_mhc_combine as sparkserve_mhc_combine_launch,
    sparkserve_qwen_runtime_mhc_mix as sparkserve_mhc_mix_launch,
    sparkserve_qwen_runtime_moe_gate as sparkserve_moe_gate_launch,
    sparkserve_qwen_runtime_moe_join as sparkserve_moe_join_launch,
    sparkserve_qwen_runtime_moe_dispatch as sparkserve_moe_route_dispatch,
    sparkserve_qwen_runtime_moe_finalize as sparkserve_moe_route_finalize,
    sparkserve_qwen_runtime_ple_gather as sparkserve_ple_gather_launch,
    sparkserve_qwen_add_hyper_launch,
    sparkserve_qwen_runtime_bf16_to_f32 as sparkserve_qwen_bf16_to_f32_launch,
    sparkserve_qwen_lm_head_launch,
    sparkserve_qwen_ple_block_launch, sparkserve_qwen_qsa_finish_launch,
    sparkserve_qwen_qsa_project_launch,
    sparkserve_qwen_repeat_embedding_launch,
    sparkserve_qwen_runtime_segmented_quantize as sparkserve_segmented_nvfp4_quantize_launch,
    sparkserve_qwen_runtime_segmented_silu as sparkserve_segmented_silu_nvfp4_launch,
    sparkserve_qwen_runtime_shared_expert as sparkserve_shared_expert_launch,
    sparkserve_qsa_decode_launch, sparkserve_qsa_expand_launch,
    sparkserve_qsa_index_prep_launch, sparkserve_qsa_kv_pack_launch,
...
use sparkserve_runtime::routing::RoutePlan;
use sparkserve_runtime::storage::{FixedPleCache, PleIndex};
use sparkserve_runtime::tokenizer::{NativeQwenTokenizer, QWEN_MODEL_MAX_LENGTH};
...
impl QwenNativeEngine {
    pub fn create(model_root: &Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if std::env::var_os("SPARKSERVE_QWEN_WARM_EXPERT_SOURCE").is_some() {
...
fn run_layer(
    ...
    native("Qwen attention mHC mix", unsafe {
        sparkserve_mhc_mix_launch(caps, &attn_mhc)
    })?;
    ...
    native("Qwen attention mHC combine", unsafe {
        sparkserve_mhc_combine_launch(caps, &attn_mhc)
    })?;
    ...
    native("Qwen MLP mHC mix", unsafe {
        sparkserve_mhc_mix_launch(caps, &mlp_mhc)
    })?;
    ...
    native("Qwen MLP mHC combine", unsafe {
        sparkserve_mhc_combine_launch(caps, &mlp_mhc)
    })?;
...
fn run_gdn(
    ...
    native("Qwen GDN prepare", unsafe { sparkserve_gdn_block_prepare_launch(caps, &block) })?;
    native("Qwen GDN recurrence", unsafe { sparkserve_gdn_decode_launch(caps, &recurrence) })?;
    native("Qwen GDN finish", unsafe { sparkserve_gdn_block_finish_launch(caps, &block) })?;
    Ok(())
}
...
fn run_qsa(
    ...
    native("Qwen QSA project", unsafe { sparkserve_qwen_qsa_project_launch(&project) })?;
    ...
    native("Qwen QSA index score", unsafe {
        sparkserve_qsa_score_launch(caps, &score)
    })?;
    ...
    native("Qwen QSA block top-k", unsafe {
        sparkserve_qsa_topk_launch(caps, &topk)
    })?;
    ...
    native("Qwen QSA token expansion", unsafe {
        sparkserve_qsa_expand_launch(caps, &expand)
    })?;
    ...
    native("Qwen QSA selected K/V pack", unsafe {
        sparkserve_qsa_kv_pack_launch(caps, &pack)
    })?;
    ...
    native("Qwen QSA sparse decode", unsafe {
        sparkserve_qsa_decode_launch(caps, &decode)
    })?;
    ...
    native("Qwen QSA finish", unsafe { sparkserve_qwen_qsa_finish_launch(&finish) })?;
    Ok(())
}
...
fn run_moe(
    ...
    native("Qwen router", unsafe { sparkserve_moe_gate_launch(caps, &gate) })?;
    stream.synchronize()?;
...
fn run_moe_token(
    ...
    *expert_packs = expert_packs
    ...
    native("Qwen route dispatch", unsafe { sparkserve_moe_route_dispatch(caps, &route_args) })?;
    ...
    native("Qwen routed quantize", unsafe { sparkserve_segmented_nvfp4_quantize_launch(caps, &quantize) })?;
    ...
    native("Qwen grouped gate/up", unsafe { sparkserve_grouped_nvfp4_launch(caps, &w13) })?;
    ...
    native("Qwen SiLU quantize", unsafe { sparkserve_segmented_silu_nvfp4_launch(caps, &silu) })?;
    ...
    native("Qwen grouped down", unsafe { sparkserve_grouped_nvfp4_launch(caps, &w2) })?;
    trace_stage(stream, trace, layer, "MoE grouped down")?;
    native("Qwen route finalize", unsafe { sparkserve_moe_route_finalize(caps, &route_args) })?;
    ...
    native("Qwen shared expert", unsafe { sparkserve_shared_expert_launch(caps, &shared) })?;
    ...
    native("Qwen MoE join", unsafe { sparkserve_moe_join_launch(caps, &join) })?;
...
fn run_ple(
    ...
    native("Qwen PLE gather", unsafe { sparkserve_ple_gather_launch(caps, &gather) })?;
    ...
    native("Qwen PLE block", unsafe { sparkserve_qwen_ple_block_launch(&ple) })?;
    glue("Qwen PLE add", arena.ple_delta.device_address(), hyper, stream, sparkserve_qwen_add_hyper_launch)?;
    stream.synchronize()?;
    Ok(())
}
...
fn finish_logits(
    ...
    native("Qwen final hyper mix", unsafe { sparkserve_mhc_mix_launch(caps, &args) })?;
    ...
    native("Qwen LM head", unsafe { sparkserve_qwen_lm_head_launch(&head) })?;
    stream.synchronize()?;
...
fn convert_bf16(stream: &CudaStreamOwner, input: u64, output: &CoherentRegionOwner, elements: u64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    native("Qwen BF16 conversion", unsafe { sparkserve_qwen_bf16_to_f32_launch(&args) })
}
