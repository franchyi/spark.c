...
| Qwen3.8-27B BF16 LM head | ds4 `glm53_matvec_bf16_f32_kernel` at `a60a2a0` (MIT) plus SGLang full-logit oracle | fixed `[248320,5120]` one-row-per-warp streaming GEMV with paired BF16 loads and FP32 accumulation | one allocation-free raw CUDA ABI; the resident 2.543-GB LM-head arena and logits remain model-owned | passed: 10.815 ms versus 15.236 ms cuBLAS on the exact SGLang hidden (1.409x); native/SGLang top-10 10/10; raw max cuBLAS drift `9.54e-6`; 248,305/248,320 oracle BF16-rounded logits exact |
