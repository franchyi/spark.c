...
| Flash-Next router/top-k | SGLang Qwen4-exp and `moe_topk_softmax.cuh` at `d91c368` | cuBLAS BF16 router GEMM plus SGLang's workspace-free 512-expert warp top-k | one raw CUDA boundary; Rust owns the long-lived handle, coherent buffers, completion, and scheduling | passed: real layer-0 eight-token BF16 logits, exact top-10 ids, and normalized FP32 weights have zero error at 16.86 us; joined shared/routed output next |
| Flash-Next shared expert | SGLang BF16 activation and sigmoid-broadcast at `d91c368` | cuBLAS merged gate/up and down plus adapted SGLang vector elementwise kernels | framework-free raw CUDA; loader builds one final merged resident weight region | passed: every real layer-0 intermediate and final BF16 byte matches for eight tokens at 30.69 us |
