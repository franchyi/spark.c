...
| QSA indexer and sparse attention | SGLang QSA backend at `7c66045` + FlashInfer `906181e` | SGLang fused Q/K prep, radix top-k, and selected-K/V pack, plus FlashInfer XQA BF16 paged decode | all four donors now share one framework-free library; Rust owns coherent ranges, streams/events, fixed page tables/workspace, and graph scheduling | Rust/coherent-memory prep, top-k, pack, and XQA fixtures pass exact parity; score GEMM, block-to-token expansion, and joined layer parity next |
