...
int main(int argc, char** argv) {
  ...
  FlashKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(flash_qsa_topk_query(&caps, &plan, &info).code ==
         FLASH_STATUS_OK);
  assert(info.available == 1);

  FlashQsaTopkArgs args = {};
  ...
  FlashStatus status = flash_qsa_topk_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = flash_qsa_topk_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
  }
