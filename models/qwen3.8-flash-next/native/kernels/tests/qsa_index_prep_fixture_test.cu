...
int main(int argc, char** argv) {
  ...
  FlashQsaIndexPrepPlan plan = {
      ...
      1,
      FLASH_DTYPE_BF16,
      FLASH_BACKEND_SGLANG_QSA_INDEX_PREP,
      kQueryHeadsPadded,
  };
  FlashKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(flash_qsa_index_prep_query(&caps, &plan, &info).code ==
         FLASH_STATUS_OK);
  assert(info.available == 1);

  FlashQsaIndexPrepArgs args = {};
  ...
  args.eps = 1.0e-6f;
  FlashStatus status = flash_qsa_index_prep_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = flash_qsa_index_prep_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
  }
