...
int main(int argc, char** argv) {
  ...
  FlashQsaExpandPlan plan = {
      ...
      kFinalTopk,
      FLASH_DTYPE_INT32,
      FLASH_BACKEND_SGLANG_QSA_EXPAND,
      0,
  };
  FlashKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(flash_qsa_expand_query(&caps, &plan, &info).code ==
         FLASH_STATUS_OK);
  assert(info.available == 1);

  FlashQsaExpandArgs args = {};
  ...
  FlashStatus status = flash_qsa_expand_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = flash_qsa_expand_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
  }
