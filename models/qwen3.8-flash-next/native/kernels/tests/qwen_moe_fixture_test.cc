...
FlashGroupedNvfp4Args GroupedArgs(
...
int FlashRunQwenMoeFixture(const char* fixture_path,
  ...
  if (with_mhc) {
    ...
    mhc.abi_version = FLASH_KERNEL_ABI_VERSION;
    mhc.plan = {sizeof(FlashMhcPlan),
                FLASH_KERNEL_ABI_VERSION,
                num_tokens,
                4,
                kHidden,
    ...
    FlashDeviceCaps mhc_caps = {
        sizeof(FlashDeviceCaps), FLASH_KERNEL_ABI_VERSION, 121, 1, 0};
    FlashStatus mhc_status = flash_mhc_mix_launch(&mhc_caps, &mhc);
    if (mhc_status.code != FLASH_STATUS_OK)
      std::cerr << mhc_status.message << '\n';
    assert(mhc_status.code == FLASH_STATUS_OK);
  ...
  FlashDeviceCaps caps = {
      sizeof(FlashDeviceCaps), FLASH_KERNEL_ABI_VERSION, 121, 1, 0};
  FlashMoeRouteArgs route = {};
  route.struct_size = sizeof(route);
  route.abi_version = FLASH_KERNEL_ABI_VERSION;
  route.plan = {
  ...
  FlashStatus status = flash_moe_route_dispatch(&caps, &route);
  ...
  FlashSegmentedNvfp4QuantizeArgs quantize = {};
  quantize.struct_size = sizeof(quantize);
  quantize.abi_version = FLASH_KERNEL_ABI_VERSION;
  quantize.plan = {
  ...
  status = flash_segmented_nvfp4_quantize_launch(&caps, &quantize);
  if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == FLASH_STATUS_OK);
  CudaOk(cudaDeviceSynchronize());
  ...
  status = flash_grouped_nvfp4_launch(&caps, &gateup_args);
  if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == FLASH_STATUS_OK);
  CudaOk(cudaDeviceSynchronize());
  ExpectBytes(gateup, gateup_expected, "grouped gate/up");

  FlashSegmentedSiluNvfp4Args activation = {};
  activation.struct_size = sizeof(activation);
  activation.abi_version = FLASH_KERNEL_ABI_VERSION;
  activation.plan = {
      ...
      FLASH_DTYPE_BF16,
      FLASH_SCALE_LAYOUT_CUTLASS_128X4,
      FLASH_BACKEND_FLASHINFER_CUTE_SILU_NVFP4,
      0,
  };
  ...
  status = flash_segmented_silu_nvfp4_launch(&caps, &activation);
  if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == FLASH_STATUS_OK);
  CudaOk(cudaDeviceSynchronize());
  ...
  status = flash_grouped_nvfp4_launch(&caps, &down_args);
  if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == FLASH_STATUS_OK);
  CudaOk(cudaDeviceSynchronize());
  ...
  status = flash_moe_route_finalize(&caps, &route);
  if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == FLASH_STATUS_OK);
  CudaOk(cudaDeviceSynchronize());
  ...
  if (joined) {
    ...
    FlashMoeGateArgs gate = {};
    gate.struct_size = sizeof(gate);
    gate.abi_version = FLASH_KERNEL_ABI_VERSION;
    gate.plan = {sizeof(FlashMoeGatePlan),
                 FLASH_KERNEL_ABI_VERSION,
                 num_tokens,
                 kHidden,
                 512,
                 ...
                 FLASH_DTYPE_BF16,
                 FLASH_DTYPE_BF16,
                 FLASH_BACKEND_SGLANG_CUBLAS_MOE_GATE,
                 1,
                 0};
    ...
    status = flash_moe_gate_launch(&caps, &gate);
    if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
    assert(status.code == FLASH_STATUS_OK);
    CudaOk(cudaDeviceSynchronize());
    ...
    FlashSharedExpertArgs shared = {};
    shared.struct_size = sizeof(shared);
    shared.abi_version = FLASH_KERNEL_ABI_VERSION;
    shared.plan = {sizeof(FlashSharedExpertPlan),
                   FLASH_KERNEL_ABI_VERSION,
                   num_tokens,
                   kHidden,
                   kMoe,
                   ...
                   FLASH_SHARED_EXPERT_OUTPUT_UNGATED,
                   0,
                   0};
    ...
    status = flash_shared_expert_launch(&caps, &shared);
    if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
    assert(status.code == FLASH_STATUS_OK);
    CudaOk(cudaDeviceSynchronize());
    ...
    FlashMoeJoinArgs join = {};
    join.struct_size = sizeof(join);
    join.abi_version = FLASH_KERNEL_ABI_VERSION;
    join.plan = {sizeof(FlashMoeJoinPlan),
                 FLASH_KERNEL_ABI_VERSION,
                 num_tokens,
                 kHidden,
                 FLASH_DTYPE_BF16,
    ...
    status = flash_moe_join_launch(&caps, &join);
    if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
    assert(status.code == FLASH_STATUS_OK);
    CudaOk(cudaDeviceSynchronize());
    ...
    if (with_mhc) {
      ...
      status = flash_mhc_combine_launch(&caps, &mhc);
    ...
    if (benchmark) {
      ...
      for (int iteration = 0; iteration < kIterations; ++iteration) {
        if (with_mhc)
          assert(flash_mhc_mix_launch(&caps, &mhc).code ==
                 FLASH_STATUS_OK);
        assert(flash_moe_gate_launch(&caps, &gate).code ==
               FLASH_STATUS_OK);
        assert(flash_moe_route_dispatch(&caps, &route).code ==
               FLASH_STATUS_OK);
        assert(
            flash_segmented_nvfp4_quantize_launch(&caps, &quantize).code ==
            FLASH_STATUS_OK);
        assert(flash_grouped_nvfp4_launch(&caps, &gateup_args).code ==
               FLASH_STATUS_OK);
        assert(
            flash_segmented_silu_nvfp4_launch(&caps, &activation).code ==
            FLASH_STATUS_OK);
        assert(flash_grouped_nvfp4_launch(&caps, &down_args).code ==
               FLASH_STATUS_OK);
        assert(flash_moe_route_finalize(&caps, &route).code ==
               FLASH_STATUS_OK);
        assert(flash_shared_expert_launch(&caps, &shared).code ==
               FLASH_STATUS_OK);
        assert(flash_moe_join_launch(&caps, &join).code ==
               FLASH_STATUS_OK);
        if (with_mhc)
          assert(flash_mhc_combine_launch(&caps, &mhc).code ==
                 FLASH_STATUS_OK);
      }
...
#ifndef FLASH_FIXTURE_LIBRARY
int main(int argc, char** argv) {
  assert(argc == 2);
  return FlashRunQwenMoeFixture(argv[1], nullptr, true);
}
#endif
