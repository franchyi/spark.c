...
int main(int argc, char** argv) {
  ...
  FlashKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(flash_moe_gate_query(&caps, &plan, &info).code ==
         FLASH_STATUS_OK);
  assert(info.available == 1);

  FlashMoeGateArgs args = {};
  ...
  FlashStatus status = flash_moe_gate_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = flash_moe_gate_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
  }
