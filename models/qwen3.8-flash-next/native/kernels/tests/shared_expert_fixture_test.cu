...
int main(int argc, char** argv) {
  ...
  FlashSharedExpertPlan plan = {
      ...
      FLASH_SHARED_EXPERT_OUTPUT_GATED,
      0,
      0,
  };
  FlashSharedExpertArgs args = {};
  ...
  FlashStatus status = flash_shared_expert_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = flash_shared_expert_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
  }
