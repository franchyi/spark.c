...
int FlashRunQwenGdnBlockFixture(const char* fixture_path,
  ...
  FlashGdnBlockArgs block = {};
  block.struct_size = sizeof(block);
  block.abi_version = FLASH_KERNEL_ABI_VERSION;
  block.plan = {sizeof(FlashGdnBlockPlan),
                FLASH_KERNEL_ABI_VERSION,
                tokens,
                kHidden,
                kQkHeads,
  ...
  FlashGdnDecodeArgs recurrence = {};
  recurrence.struct_size = sizeof(recurrence);
  recurrence.abi_version = FLASH_KERNEL_ABI_VERSION;
  recurrence.plan = {sizeof(FlashGdnDecodePlan),
                     FLASH_KERNEL_ABI_VERSION,
                     1,
                     kQkHeads,
                     kValueHeads,
  ...
  Check(flash_mhc_mix_launch(&caps, &mhc));
  Check(flash_gdn_block_prepare_launch(&caps, &block));
  Check(flash_gdn_decode_launch(&caps, &recurrence));
  Check(flash_gdn_block_finish_launch(&caps, &block));
  Check(flash_mhc_combine_launch(&caps, &mhc));
...
int main(int argc, char** argv) {
  assert(argc == 2);
  return FlashRunQwenGdnBlockFixture(argv[1], nullptr, true);
}
#endif
