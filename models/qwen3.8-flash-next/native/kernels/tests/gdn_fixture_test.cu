...
int main(int argc, char** argv) {
  ...
  FlashGdnDecodePlan plan = {
      ...
      kHeadDim,                         1,
      FLASH_DTYPE_BF16,            FLASH_GDN_BACKEND_FLASHINFER};
  auto* qkv_bytes = static_cast<uint8_t*>(qkv_device);
  FlashGdnDecodeArgs args = {
      sizeof(FlashGdnDecodeArgs), FLASH_KERNEL_ABI_VERSION,
      plan,                             qkv_bytes,
      ...
      1,
      nullptr};
  FlashStatus status = flash_gdn_decode_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kBenchmarkIterations; ++iteration) {
    ...
    status = flash_gdn_decode_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
