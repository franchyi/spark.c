...
int main() {
  ...
  constexpr uint32_t kCapacity = FLASH_QWEN_EXPERT_CAPACITY;
  constexpr size_t kGateWeightBytes = FLASH_QWEN_W13_WEIGHT_BYTES / 2;
  constexpr size_t kDownWeightBytes = FLASH_QWEN_W2_WEIGHT_BYTES;
  constexpr size_t kGateScaleBytes = FLASH_QWEN_W13_SCALE_BYTES / 2;
  constexpr size_t kDownScaleBytes = FLASH_QWEN_W2_SCALE_BYTES;
  ...
  uint8_t* w13_weights =
      Allocate<uint8_t>(kCapacity * FLASH_QWEN_W13_WEIGHT_BYTES);
  uint8_t* w2_weights =
      Allocate<uint8_t>(kCapacity * FLASH_QWEN_W2_WEIGHT_BYTES);
  uint8_t* w13_scales =
      Allocate<uint8_t>(kCapacity * FLASH_QWEN_W13_SCALE_BYTES);
  uint8_t* w2_scales =
      Allocate<uint8_t>(kCapacity * FLASH_QWEN_W2_SCALE_BYTES);
  ...
  Require(flash_qwen_expert_pack_launch(&args));
