...
int main(int argc, char** argv) {
  ...
  FlashQsaKvPackPlan plan = {
      ...
      kHeadDim,
      FLASH_DTYPE_BF16,
      FLASH_BACKEND_SGLANG_QSA_KV_PACK,
  };
  FlashKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(flash_qsa_kv_pack_query(&caps, &plan, &info).code ==
         FLASH_STATUS_OK);
  assert(info.available == 1);

  FlashQsaKvPackArgs args = {};
  ...
  FlashStatus status = flash_qsa_kv_pack_launch(&caps, &args);
  ...
  for (int iteration = 0; iteration < kIterations; ++iteration) {
    status = flash_qsa_kv_pack_launch(&caps, &args);
    assert(status.code == FLASH_STATUS_OK);
  }
