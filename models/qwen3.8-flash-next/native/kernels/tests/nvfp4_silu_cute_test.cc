...
int main(int argc, char** argv) {
  ...
  FlashKernelInfo info = {
      ...
      0,                             nullptr,
      nullptr};
  assert(flash_silu_nvfp4_query(&caps, &plan, &info).code ==
         FLASH_STATUS_OK);
  assert(info.available == 1);

  FlashSiluNvfp4Args args = {};
  ...
  FlashStatus status = flash_silu_nvfp4_launch(&caps, &args);
  ...
  FlashSegmentedSiluNvfp4Plan segmented_plan = {
  ...
  status = flash_segmented_silu_nvfp4_launch(&caps, &segmented_args);
  if (status.code != FLASH_STATUS_OK) std::cerr << status.message << '\n';
  assert(status.code == FLASH_STATUS_OK);
  CudaOk(cudaDeviceSynchronize());
  CudaOk(cudaMemcpy(host_output.data(), output, host_output.size(),
