...
FlashStatus Bind(cublasHandle_t handle, cudaStream_t stream) {
...
extern "C" FlashStatus flash_qwen_qsa_project_launch(
    const FlashQwenQsaProjectArgs* args) {
  if (args == nullptr || args->struct_size != sizeof(*args) ||
      args->abi_version != FLASH_QWEN_QSA_BLOCK_ABI_VERSION ||
...
extern "C" FlashStatus flash_qwen_qsa_finish_launch(
