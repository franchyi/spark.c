...
use spark_flash_next::ffi::{
    ...
    QwenPleBlockArgs, Status, flash_ple_gather_launch, flash_qwen_ple_block_launch,
};
use spark_flash_next::kernel::KERNEL_ABI_VERSION;
use spark_flash_next::qwen_ple::decode_row_ids;
use spark_flash_next::qwen_weights::{FlashNextWeightMaps, QwenTensorView};
use spark_flash_next::storage::{FixedPleCache, PleIndex};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ple_index = PleIndex::decode(&std::fs::read(
        model_path.join(".spark.c/ple.ssple"),
    )?)?;
    ...
    native("Qwen real PLE block", unsafe {
        flash_qwen_ple_block_launch(&args)
    })?;
    stream.synchronize()?;
