...
use spark_flash_next::qwen_expert_sidecar::{
    ExpertSidecarProgress, build_expert_sidecar, verify_expert_sidecar,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let report = if mode.as_deref() == Some("--verify") {
        verify_expert_sidecar(&checkpoint, Path::new(&output), print_progress)?
    } else {
        build_expert_sidecar(&checkpoint, Path::new(&output), print_progress)?
    };
    println!(
        "Qwen expert sidecar ready: records={} bytes={} resumed={} existing={} fingerprint={}",
        report.records,
        report.bytes,
...
fn print_progress(progress: ExpertSidecarProgress) {
    ...
    eprintln!(
        "Qwen expert sidecar: {}/{} ({percent:.1}%, resumed {})",
...
fn usage() -> ! {
    eprintln!(
        "usage: qwen_expert_sidecar <model-root> <output.ssx> [--verify]\n\
