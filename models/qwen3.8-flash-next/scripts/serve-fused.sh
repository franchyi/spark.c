...
pid_file="$engine_dir/.native-fused.pid"
...
for required in \
  ...
  "$repo_root/build/flash-next/libflash-qwen-fused-moe.so" \
  "$repo_root/build/flash-next/libflash-qwen-runtime-fused.so" \
...
if [[ -f "$pid_file" ]]; then
  ...
  if kill -0 "$old_pid" 2>/dev/null; then
    echo "fused Flash-Next server already running as pid $old_pid" >&2
    exit 1
  fi
  rm -f "$pid_file"
fi
