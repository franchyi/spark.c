...
target_host=${FLASH_RUST_FUSED_TARGET:-${HOME}/.cache/flash-rust-target-fused}
cargo_home=${FLASH_CARGO_HOME:-${HOME}/.cache/flash-cargo-home}
cuda_host_root=${FLASH_CUDA_HOST_ROOT:-$(readlink -f /usr/local/cuda)}
user_id=$(id -u)
group_id=$(id -g)
native_dir="$repo_root/build/flash-next"

for required in \
  "$native_dir/libflash-fabric.so" \
  "$native_dir/libflash-qwen-runtime.so" \
  "$native_dir/libflash-qwen-fused-moe.so" \
  "$native_dir/libflash-qwen-runtime-fused.so" \
  "$native_dir/libtvm_ffi.so"; do
  if [[ ! -e "$required" ]]; then
...
cp "$target_host/release/examples/qwen_serve" \
