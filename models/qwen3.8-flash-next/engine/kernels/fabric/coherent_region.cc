...
FlashStatus ErrnoStatus(const char* prefix) {
...
FlashStatus ValidateHeader(uint32_t struct_size, uint32_t expected,
                                uint32_t abi_version) {
  if (struct_size != expected) return Invalid("coherent region struct size mismatch");
  if (abi_version != FLASH_FABRIC_ABI_VERSION) {
...
FlashStatus MapAnonymous(const FlashCoherentRegionConfig* config,
  ...
  if ((config->flags & FLASH_COHERENT_REGION_HUGE_PAGE_HINT) != 0) {
  ...
  if ((config->flags & FLASH_COHERENT_REGION_PREFAULT) != 0) {
    PrefaultWritable(host_base, mapped_bytes, region->page_bytes);
  }
  return Ok();
}

FlashStatus MapFile(const FlashCoherentRegionConfig* config,
                         FlashCoherentRegion* region) {
  int fd = open(config->file_path, O_RDONLY | O_CLOEXEC);
  if (fd < 0) return ErrnoStatus("coherent file open failed: ");
  struct stat stat_buffer = {};
  if (fstat(fd, &stat_buffer) != 0) {
    const FlashStatus status = ErrnoStatus("coherent file stat failed: ");
    close(fd);
    return status;
  }
  uint64_t file_end = 0;
...
void Unmap(FlashCoherentRegion* region) {
...
extern "C" FlashStatus flash_coherent_region_validate(
    const FlashCoherentRegionConfig* config) {
  if (config == nullptr) return Invalid("coherent region config is required");
  FlashStatus header = ValidateHeader(config->struct_size, sizeof(*config),
                                           config->abi_version);
  if (header.code != FLASH_STATUS_OK) return header;
  constexpr uint32_t kKnownFlags = FLASH_COHERENT_REGION_PREFAULT |
...
extern "C" FlashStatus flash_coherent_region_create(
    const FlashCoherentRegionConfig* config,
    FlashCoherentRegion** output) {
  FlashStatus status = flash_coherent_region_validate(config);
  ...
  *output = nullptr;
  ...
  if (page_size <= 0 || !IsPowerOfTwo(static_cast<uint64_t>(page_size))) {
    return {FLASH_STATUS_INTERNAL, "cannot resolve host page size"};
  }
  auto* region = new (std::nothrow) FlashCoherentRegion;
  if (region == nullptr) {
    return {FLASH_STATUS_INTERNAL, "cannot allocate coherent region handle"};
  }
  ...
  status = config->kind == FLASH_COHERENT_REGION_SLAB
...
extern "C" FlashStatus flash_coherent_region_view(
...
extern "C" FlashStatus flash_coherent_region_destroy(
    FlashCoherentRegion* region) {
