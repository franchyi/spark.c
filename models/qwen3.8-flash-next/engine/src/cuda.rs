...
use crate::ffi::{
    CudaBlas, CudaEvent, CudaStream, Status, flash_cuda_blas_create,
    flash_cuda_blas_destroy, flash_cuda_blas_raw, flash_cuda_event_create,
    flash_cuda_event_destroy, flash_cuda_event_query, flash_cuda_event_record,
    flash_cuda_event_synchronize, flash_cuda_stream_create,
    flash_cuda_stream_destroy, flash_cuda_stream_memset_async,
    flash_cuda_stream_memcpy_async, flash_cuda_stream_raw,
    flash_cuda_stream_synchronize,
    flash_cuda_stream_wait_event,
};
...
impl CudaBlasOwner {
    pub fn create() -> Result<Self, CudaRuntimeError> {
        ...
        status_result(unsafe { flash_cuda_blas_create(&mut raw_owner) })?;
        let handle =
        ...
        if let Err(error) =
            status_result(unsafe { flash_cuda_blas_raw(handle.as_ptr(), &mut raw_blas) })
        {
            // SAFETY: this function still uniquely owns the handle.
            let _ = unsafe { flash_cuda_blas_destroy(handle.as_ptr()) };
            return Err(error);
        }
    ...
    fn destroy(&mut self) -> Result<(), CudaRuntimeError> {
        ...
        status_result(unsafe { flash_cuda_blas_destroy(handle.as_ptr()) })
    }
}
...
impl CudaStreamOwner {
    pub fn create() -> Result<Self, CudaRuntimeError> {
        let mut raw_owner = std::ptr::null_mut();
        // SAFETY: output storage is valid and unique.
        status_result(unsafe { flash_cuda_stream_create(&mut raw_owner) })?;
        let handle =
        ...
        if let Err(error) =
            status_result(unsafe { flash_cuda_stream_raw(handle.as_ptr(), &mut raw_stream) })
        {
            // SAFETY: this function still uniquely owns the handle.
            let _ = unsafe { flash_cuda_stream_destroy(handle.as_ptr()) };
            return Err(error);
        }
    ...
    pub unsafe fn memset_async(
        ...
        status_result(unsafe {
            flash_cuda_stream_memset_async(
                self.handle().as_ptr(),
                pointer,
                u32::from(value),
                bytes,
    ...
    pub unsafe fn memcpy_async(
        ...
        status_result(unsafe {
            flash_cuda_stream_memcpy_async(
                self.handle().as_ptr(),
    ...
    pub fn wait_event(&mut self, event: &CudaEventOwner) -> Result<(), CudaRuntimeError> {
        ...
        status_result(unsafe {
            flash_cuda_stream_wait_event(self.handle().as_ptr(), event.handle().as_ptr())
        })
    }

    pub fn synchronize(&mut self) -> Result<(), CudaRuntimeError> {
        // SAFETY: the unique owner keeps its stream handle live.
        status_result(unsafe { flash_cuda_stream_synchronize(self.handle().as_ptr()) })
    }
    ...
    fn destroy(&mut self) -> Result<(), CudaRuntimeError> {
        ...
        // SAFETY: taking the option transfers unique ownership exactly once.
        status_result(unsafe { flash_cuda_stream_destroy(handle.as_ptr()) })
    }
}
...
impl CudaEventOwner {
    pub fn create() -> Result<Self, CudaRuntimeError> {
        ...
        // SAFETY: output storage is valid and unique.
        status_result(unsafe { flash_cuda_event_create(&mut raw) })?;
        let handle =
    ...
    pub fn record(&mut self, stream: &mut CudaStreamOwner) -> Result<(), CudaRuntimeError> {
        ...
        status_result(unsafe {
            flash_cuda_event_record(self.handle().as_ptr(), stream.handle().as_ptr())
        })
    }
    ...
    pub fn query(&self) -> Result<bool, CudaRuntimeError> {
        ...
        status_result(unsafe {
            flash_cuda_event_query(self.handle().as_ptr(), &mut complete)
        })?;
        match complete {
            0 => Ok(false),
            1 => Ok(true),
    ...
    pub fn synchronize(&mut self) -> Result<(), CudaRuntimeError> {
        ...
        status_result(unsafe { flash_cuda_event_synchronize(self.handle().as_ptr()) })
    }
    ...
    fn destroy(&mut self) -> Result<(), CudaRuntimeError> {
        ...
        // SAFETY: taking the option transfers unique ownership exactly once.
        status_result(unsafe { flash_cuda_event_destroy(handle.as_ptr()) })
    }
}
