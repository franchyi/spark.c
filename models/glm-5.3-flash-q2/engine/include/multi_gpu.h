/* multi_gpu.h — multi-GPU plumbing types and APIs (v0).
 *
 * This header carries the new multi-GPU additions for the multi-GPU plumbing PP work
 * (device-aware CUDA). It is included from cuda/runtime.cu and from
 * downstream tasks that need access to g_gpu[], g_n_gpus, g_gpu_peer_ok[],
 * the gpu_config struct, and the new tensor APIs.
 *
 * Why not in gpu.h? The legacy gpu.h is included from C-only
 * callers (engine.c, cli.c, etc.) and from the Metal build, but is NOT
 * included from cuda/runtime.cu historically. That asymmetry hid pre-existing
 * signature mismatches between the legacy header and cuda/runtime.cu. We keep
 * the legacy header opaque and put the new shared types here, so this
 * file is the single source of truth for both cuda/runtime.cu and downstream
 ...
 * builds, and the CUDA build (where cuda/runtime.cu casts the void * back
...
/* Complete definition of the previously-opaque gpu_tensor, plus a
 * typedef so the new API prototypes below can use the bare name
 * `gpu_tensor *` in both C and C++ without forcing callers to
 * include gpu.h first. Callers that include this header can
...
/* Tagged so headers (notably engine.h) can forward-declare `struct
 * gpu_config` without dragging in this entire header. */
...
extern gpu_ctx g_gpu[DS4_MAX_GPUS];
extern int         g_n_gpus;
extern int         g_gpu_peer_ok[DS4_MAX_GPUS][DS4_MAX_GPUS];

/* Primary multi-device init. The existing gpu_init (declared in
 * gpu.h) is a thin shim that builds a single-device config for
 * device 0 and calls this. */
int gpu_init_multi(const gpu_config *cfg);

/* Caller-supplied struct alloc on a specific device. Returns 0 on
 * success, nonzero on error. Pair with gpu_tensor_free_in_place. */
int  gpu_tensor_alloc_on(gpu_tensor *t, int device_id, uint64_t bytes);
void gpu_tensor_free_in_place(gpu_tensor *t);

/* Heap-allocated tensor on a specific logical tier; mirrors the legacy
 * gpu_tensor_alloc ABI (returns gpu_tensor *) but with a tier
 * parameter. Returns NULL on failure. Used by the multi-tier graph
 * allocations in engine.c. Single-tier callers can continue using the
 * legacy gpu_tensor_alloc(bytes) which is equivalent to
 * gpu_tensor_alloc_ptr_on(0, bytes). */
gpu_tensor *gpu_tensor_alloc_ptr_on(int tier, uint64_t bytes);
int gpu_tensor_copy_async(gpu_tensor *dst, const gpu_tensor *src, uint64_t bytes);
void gpu_enable_q8_dequant_gemm(void);
 ...
 * gpu_tensor_free runs under the correct device. Note managed memory
 * pages on first-touch and is not strictly device-bound; the stamped
 * tier records the home tier for accounting + free. */
gpu_tensor *gpu_tensor_alloc_managed_on(int tier, uint64_t bytes);
...
int gpu_tensor_copy_xdev(gpu_tensor *dst,
                              const gpu_tensor *src,
                              uint64_t bytes);
int gpu_tensor_copy_xdev_default(gpu_tensor *dst,
                                      const gpu_tensor *src,
                                      uint64_t bytes);
...
int gpu_tensor_copy_xdev3_default_dst(
        gpu_tensor       *dst0,
        const gpu_tensor *src0,
        uint64_t              bytes0,
        gpu_tensor       *dst1,
        const gpu_tensor *src1,
        uint64_t              bytes1,
        gpu_tensor       *dst2,
        const gpu_tensor *src2,
        uint64_t              bytes2);
 ...
 * gpu_tensor_copy_xdev calls would spend more time in event plumbing than
 * in the copies themselves. Falls back internally when a grouped peer copy is
 * not applicable. */
int gpu_tensor_copy_xdev3(gpu_tensor       *dst0,
                              const gpu_tensor *src0,
                              uint64_t              bytes0,
                              gpu_tensor       *dst1,
                              const gpu_tensor *src1,
 ...
 * stage just consumed. */
int gpu_tensor_copy_xdev_ordered(gpu_tensor *dst,
                                      const gpu_tensor *src,
                                      uint64_t bytes);

/* Order a peer read without copying: records readiness on src's stream and
 * makes dst_tier's stream wait for it. */
int gpu_tensor_wait_xdev(const gpu_tensor *src, int dst_tier);
int gpu_tensor_wait_xdev_default(const gpu_tensor *src, int dst_tier);

/* Cross-device float add used by CUDA tensor-parallel reductions.
 * See gpu.h for the full contract. */
int gpu_add_xdev_tensor(gpu_tensor       *out,
                             const gpu_tensor *local,
                             const gpu_tensor *remote,
                             gpu_tensor       *remote_tmp,
                             uint32_t              n);

/* Returns the device_id recorded on the tensor; -1 if untagged. */
int gpu_tensor_device(const gpu_tensor *t);
...
int gpu_set_current_device(int logical_tier);
int gpu_set_current_device_fenced(int logical_tier);
 ...
 * gpu_set_model_map normally honors, which is essential for
...
int gpu_register_model_map_no_copy(const void *model_map, uint64_t model_size);
 ...
 * keep using gpu_lookup_cache for back-compat behavior. */
int gpu_lookup_cache_strict(uint64_t source_offset,
                                 uint64_t bytes,
                                 int      expected_device,
                                 void   **out_device_ptr);
