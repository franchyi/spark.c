...
typedef struct {
    ...
    int leader_port;
    tensor_parallel_transport transport;
 ...
 *
 * Accepts an optional gpu_config (defined in multi_gpu.h) that
...
/* gpu_config is declared in multi_gpu.h, which callers should
...
/* Decode gate schedule for the TP transport; see tensor_parallel_identity. */
 ...
 * tensor_parallel_create() and before any session work.  Transport lifecycle stays
 * with the caller. */
struct tensor_parallel;
int ds4_engine_tp_bind(ds4_engine *e, struct tensor_parallel *tp, char *err, size_t errlen);
 ...
 * (keep, or roll back and replay). Only called from tensor_parallel_worker_run. */
 ...
 * transport/session routing logic lives in distributed.c. */
