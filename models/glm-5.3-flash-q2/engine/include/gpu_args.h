/* gpu_args.h — shared CLI argument parser for --gpu-vram and
 ...
 * "auto" path is gated and dispatches to gpu_args_probe_auto_cuda,
 * which is defined in cuda/runtime.cu. On Metal/CPU builds, "auto" returns
 * an error.
 */
 ...
 */
int format_gpu_layout_line(const gpu_config *cfg,
                           bool                  was_auto,
                           char                 *out,
                           size_t                outlen);

/* CUDA-only auto probe. Defined in cuda/runtime.cu under the same gate.
...
int gpu_args_probe_auto_cuda(const int      *device_filter,
                                  int             filter_len,
                                  gpu_config *out,
