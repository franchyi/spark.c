/* gpu_args.c — shared parser for --gpu-vram and --gpu-devices.
 *
 * See gpu_args.h. The actual CUDA probing for the "auto" branch
 * lives in cuda/runtime.cu so this .c file never includes <cuda_runtime.h>
...
int parse_gpu_vram_arg(const char     *vram_arg,
    ...
    *out_skip_cuda = false;
    ...
    if (!strcmp(vram_arg, "auto")) {
        ...
        if (gpu_args_probe_auto_cuda(filter_ptr, dev_count, out,
