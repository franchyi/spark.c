/* Tensor-parallel transport and lockstep protocol.  See tensor_parallel.h and
 ...
 * note at tensor_parallel_rdma) or over a dedicated full-duplex TCP socket at 16KB
...
typedef struct {
    ...
    int gid_index;
    uint32_t max_inline;
    tensor_parallel_rdma_info peer;
...
} tensor_parallel_rdma;
#endif

struct tensor_parallel {
    ...
    uint64_t vec_bytes;
    uint32_t n_slots;
    /* Decode gate schedule (see tensor_parallel_identity). */
    uint32_t gate_slot_start;
    uint32_t gate_slot_step;
    uint32_t gates_per_token;
    uint64_t gate_slot_mask[DS4_TP_GATE_MASK_WORDS];
    uint8_t *slab;
    uint64_t slab_bytes;
    /* Slab regions, see tensor_parallel.h layout comment. */
    uint64_t out_off;
    uint64_t in_off;
    ...
    atomic_bool failed;
#ifdef DS4_TP_HAVE_VERBS
    tensor_parallel_rdma rdma;
#endif
};

/* ------------------------------------------------------------------------
 * Small socket helpers (same conventions as distributed.c).
 * --------------------------------------------------------------------- */
...
static int tp_peer_closed(const tensor_parallel *tp) {
    char byte;
...
static int tp_send_frame(int fd, uint32_t type, const void *payload, uint32_t bytes) {
    tensor_parallel_frame_header h = { DS4_TP_MAGIC, type, bytes };
...
static int tp_read_frame_header(int fd, uint32_t *type, uint32_t *bytes) {
    tensor_parallel_frame_header h;
...
int tensor_parallel_validate_engine_options(
        ...
        size_t errlen)
{
    if (!tensor_parallel_enabled(&opt->tp)) {
...
static void tp_slab_layout(tensor_parallel *tp) {
...
static uint32_t tp_slot(const tensor_parallel *tp, uint32_t layer, uint32_t gate) {
    (void)tp;
...
static int tp_rdma_load_api(tensor_parallel_verbs_api *api) {
...
static int tp_rdma_probe(tensor_parallel_verbs_api *api) {
...
static int tp_rdma_open(tensor_parallel *tp, char *err, size_t errlen) {
    tensor_parallel_rdma *r = &tp->rdma;
    int num = 0;
...
static int tp_rdma_post_gate_recv(tensor_parallel *tp, uint64_t seq);

static int tp_rdma_register_and_exchange(tensor_parallel *tp, char *err, size_t errlen) {
    tensor_parallel_rdma *r = &tp->rdma;
    ...
    tensor_parallel_rdma_info mine = {0};
 ...
 * and the ATTN slots. */
static uint32_t tp_gate_slot(const tensor_parallel *tp, uint64_t seq) {
...
static int tp_rdma_drain_cq(tensor_parallel *tp) {
    tensor_parallel_rdma *r = &tp->rdma;
    struct ibv_wc wc[16];
...
static int tp_rdma_gate_exchange(tensor_parallel *tp, uint32_t layer, uint32_t gate, uint64_t seq) {
    tensor_parallel_rdma *r = &tp->rdma;
...
static int tp_rdma_big_gate_capable(const tensor_parallel *tp) {
...
static int tp_rdma_drain_decode_window(tensor_parallel *tp) {
    tensor_parallel_rdma *r = &tp->rdma;
...
static int tp_rdma_big_gate_exchange(tensor_parallel *tp,
                                     const void *out,
                                     void *in,
                                     uint64_t bytes) {
    tensor_parallel_rdma *r = &tp->rdma;
...
static void tp_rdma_close(tensor_parallel *tp) {
    tensor_parallel_rdma *r = &tp->rdma;
...
static int tp_hello_exchange(tensor_parallel *tp, const tensor_parallel_identity *id, int rdma_ok,
                             char *err, size_t errlen) {
    tensor_parallel_hello_fixed mine = {
        .magic = DS4_TP_MAGIC,
    ...
    tensor_parallel_hello_fixed theirs;
...
{
    *out = NULL;
    tensor_parallel *tp = calloc(1, sizeof(*tp));
    if (!tp) {
...
int tensor_parallel_rank(const tensor_parallel *tp) { return tp->rank; }
bool tensor_parallel_is_rdma(const tensor_parallel *tp) { return tp->rdma_active; }
uint32_t tensor_parallel_peer_ctx(const tensor_parallel *tp) { return tp->peer_ctx; }
...
int tensor_parallel_gate_exchange(tensor_parallel *tp, uint32_t layer, uint32_t gate, uint64_t seq) {
    ...
    tensor_parallel_gate_header h = { DS4_TP_MAGIC, (uint16_t)layer, (uint16_t)gate, seq };
    struct iovec iov[2] = {
        { &h, sizeof(h) },
        { tp->slab + tensor_parallel_slab_out_offset(tp, layer, gate), tp->vec_bytes },
    };
    ...
    if (w < 0 || (size_t)w != want) {
        ...
        if (!tp_write_full(tp->data_fd,
                           tp->slab + tensor_parallel_slab_out_offset(tp, layer, gate) + payload_done,
                           tp->vec_bytes - payload_done))
            return 0;
    }
    tensor_parallel_gate_header ph;
    ...
    if (!tp_read_full(tp->data_fd, tp->slab + tensor_parallel_slab_in_offset(tp, layer, gate),
                      tp->vec_bytes))
        return 0;
    return 1;
}
...
int tensor_parallel_batch_gate_exchange(tensor_parallel *tp, uint32_t layer, uint32_t rows,
    ...
    tensor_parallel_gate_header h = { DS4_TP_BATCH_MAGIC, (uint16_t)layer,
                             (uint16_t)rows, seq };
#ifdef DS4_TP_HAVE_VERBS
    if (tp->rdma_active && tp_rdma_big_gate_capable(tp)) {
        ...
        return tp_rdma_big_gate_exchange(
                tp,
                tp->slab + tensor_parallel_slab_batch_out_offset(tp, layer),
                tp->slab + tensor_parallel_slab_batch_in_offset(tp, layer),
                bytes);
    }
#endif
    struct iovec iov[2] = {
        { &h, sizeof(h) },
        { tp->slab + tensor_parallel_slab_batch_out_offset(tp, layer), bytes },
    };
...
int tensor_parallel_big_gate_exchange(tensor_parallel *tp, uint32_t layer, uint64_t seq,
    ...
    tensor_parallel_gate_header h = { DS4_TP_BATCH_MAGIC, (uint16_t)layer, 0xB16u, seq };
    if (!tp_write_full(tp->data_fd, &h, sizeof(h))) return 0;
    tensor_parallel_gate_header ph;
    if (!tp_read_full(tp->data_fd, &ph, sizeof(ph))) return 0;
    if (ph.magic != DS4_TP_BATCH_MAGIC || ph.layer != layer ||
...
typedef struct {
    ...
    uint32_t count;
    uint32_t reserved;
} tensor_parallel_token_command_header;

typedef struct {
    uint64_t session_id;
    int32_t value;
    uint32_t reserved;
} tensor_parallel_value_command;
...
typedef struct {
    ...
    int32_t token;
    uint32_t reserved;
} tensor_parallel_eval_command;

typedef struct {
    uint32_t count;
    uint32_t reserved;
} tensor_parallel_batch_command_header;

typedef struct {
    uint64_t prefill_session_id;
    uint32_t prompt_count;
    uint32_t item_count;
} tensor_parallel_mixed_command_header;

typedef struct {
    uint64_t session_id;
    int32_t status;
    uint32_t reserved;
} tensor_parallel_command_ack;

static int tp_send_token_command(tensor_parallel *tp, uint32_t type,
                                 uint64_t session_id, const int *tokens,
                                 uint32_t count) {
    const uint64_t bytes64 = sizeof(tensor_parallel_token_command_header) +
    ...
    if (!payload) return 0;
    tensor_parallel_token_command_header h = { session_id, count, 0 };
...
int tensor_parallel_send_session_create(tensor_parallel *tp, uint64_t session_id, int ctx_size) {
    tensor_parallel_value_command msg = { session_id, (int32_t)ctx_size, 0 };
...
int tensor_parallel_send_eval(tensor_parallel *tp, uint64_t session_id,
    ...
    tensor_parallel_eval_command msg = { session_id, seq, (int32_t)token, 0 };
...
int tensor_parallel_send_rewind(tensor_parallel *tp, uint64_t session_id, int pos) {
    tensor_parallel_value_command msg = { session_id, (int32_t)pos, 0 };
...
int tensor_parallel_send_eval_batch(tensor_parallel *tp, const tensor_parallel_batch_item *items,
                           uint32_t count) {
    const uint64_t bytes64 = sizeof(tensor_parallel_batch_command_header) +
                             (uint64_t)count * sizeof(*items);
    if (!tp || !items || count == 0 || bytes64 > UINT32_MAX) return 0;
    const uint32_t bytes = (uint32_t)bytes64;
    uint8_t *payload = malloc(bytes);
    if (!payload) return 0;
    tensor_parallel_batch_command_header h = { count, 0 };
    memcpy(payload, &h, sizeof(h));
...
int tensor_parallel_send_mixed_batch(tensor_parallel *tp, uint64_t prefill_session_id,
    ...
    const uint64_t bytes64 = sizeof(tensor_parallel_mixed_command_header) +
    ...
    uint8_t *payload = malloc(bytes);
    if (!payload) return 0;
    tensor_parallel_mixed_command_header h = {
...
int tensor_parallel_send_command_ack(tensor_parallel *tp, uint64_t session_id, int status) {
    tensor_parallel_command_ack ack = { session_id, (int32_t)status, 0 };
...
static int tp_command_decode_tokens(tensor_parallel_command *command,
                                    const uint8_t *payload,
                                    uint32_t bytes,
                                    char *err, size_t errlen) {
    if (bytes < sizeof(tensor_parallel_token_command_header)) return 0;
    tensor_parallel_token_command_header h;
...
int tensor_parallel_recv_command(tensor_parallel *tp, tensor_parallel_command *command,
                        char *err, size_t errlen) {
    memset(command, 0, sizeof(*command));
    command->type = DS4_TP_FRAME_ERROR;
    ...
    case DS4_TP_FRAME_REWIND: {
        tensor_parallel_value_command msg;
    ...
    case DS4_TP_FRAME_EVAL: {
        tensor_parallel_eval_command msg;
        if (bytes != sizeof(msg)) { ok = 0; break; }
        memcpy(&msg, payload, sizeof(msg));
        command->session_id = msg.session_id;
        command->seq = msg.seq;
    ...
    case DS4_TP_FRAME_EVAL_BATCH: {
        tensor_parallel_batch_command_header h;
        if (bytes < sizeof(h)) { ok = 0; break; }
        memcpy(&h, payload, sizeof(h));
        const uint64_t want = sizeof(h) +
                              (uint64_t)h.count * sizeof(tensor_parallel_batch_item);
    ...
    case DS4_TP_FRAME_MIXED_BATCH: {
        ...
        const uint64_t item_bytes =
            (uint64_t)h.item_count * sizeof(tensor_parallel_batch_item);
    ...
    free(payload);
    if (ok <= 0) {
        tensor_parallel_command_free(command);
        if (ok == 0) {
    ...
    command->type = (tensor_parallel_frame_type)ftype;
    return 1;
}
...
typedef struct {
    uint64_t id;
    ds4_session *session;
} tensor_parallel_worker_session;

typedef struct {
    tensor_parallel_worker_session *v;
    uint32_t len;
    uint32_t cap;
} tensor_parallel_worker_sessions;

static int tp_worker_session_index(const tensor_parallel_worker_sessions *sessions,
                                   uint64_t id) {
...
static ds4_session *tp_worker_session_find(
        const tensor_parallel_worker_sessions *sessions, uint64_t id) {
    const int index = tp_worker_session_index(sessions, id);
    return index >= 0 ? sessions->v[index].session : NULL;
}

static int tp_worker_session_add(tensor_parallel_worker_sessions *sessions,
    ...
    sessions->v[sessions->len++] = (tensor_parallel_worker_session){ id, session };
    return 1;
}

static void tp_worker_session_remove(tensor_parallel_worker_sessions *sessions,
                                     uint32_t index) {
...
static int tp_worker_send_logits(tensor_parallel *tp, ds4_session *session,
...
int tensor_parallel_worker_run(ds4_engine *engine, const tensor_parallel_options *opt) {
    ...
    tensor_parallel *tp = NULL;
    if (!tensor_parallel_create(&tp, opt, &id, err, sizeof(err))) {
    ...
    tensor_parallel_worker_sessions sessions = {0};
    ...
    while (1) {
        ...
        if (!tensor_parallel_recv_command(tp, &command, err, sizeof(err))) {
            ds4_log(stderr, DS4_LOG_ERROR, "tp worker: %s", err);
            rc = 1;
            break;
        }
        ...
        if (command.type == DS4_TP_FRAME_SESSION_CREATE) {
            ...
            if (!tensor_parallel_send_command_ack(tp, command.session_id, status)) {
                rc = 1;
            }
            tensor_parallel_command_free(&command);
            if (rc != 0) break;
        ...
        if (command.type == DS4_TP_FRAME_SESSION_DESTROY) {
            ...
            if (!tensor_parallel_send_command_ack(tp, command.session_id, status)) rc = 1;
            tensor_parallel_command_free(&command);
            if (rc != 0) break;
            continue;
        }
        ...
        if (command.type == DS4_TP_FRAME_SYNC) {
            ...
            if (!tensor_parallel_send_command_ack(tp, command.session_id, sync_rc)) {
                rc = 1;
            } else if (sync_rc != 0) {
            ...
            if (!tensor_parallel_send_command_ack(tp, command.session_id, batch_rc)) {
                rc = 1;
            } else if (batch_rc != 0) {
                ds4_log(stderr, DS4_LOG_ERROR,
