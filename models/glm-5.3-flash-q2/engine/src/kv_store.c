 ...
 * glm-server still owns the automatic byte-prefix cache policy built on top of
 * this file; ds4-agent uses only the same durable format for explicit sessions,
 * with its own policy in the agent front end. Protocol-specific extras, such as the
...
static const char *kv_log_name(const kv_store *kc) {
    return kc && kc->log_name ? kc->log_name : "ds4";
}

static void kv_logf(kv_store *kc, kv_store_log_type type,
                    const char *fmt, ...) {
    if (!kc || !kc->log) return;
    va_list ap;
    va_start(ap, fmt);
...
char *kv_store_path_for_sha(kv_store *kc, const char sha[41]) {
    ...
    memcpy(name, sha, 40);
    memcpy(name + 40, ".kv", 4);
    return kv_store_path_join(kc->dir, name);
}
...
void kv_store_clear(kv_store *kc) {
    for (int i = 0; i < kc->len; i++) kv_store_entry_free(&kc->entry[i]);
    free(kc->entry);
    kc->entry = NULL;
    kc->len = 0;
    kc->cap = 0;
...
static void kv_cache_push(kv_store *kc, kv_store_entry e) {
    if (kc->len == kc->cap) {
...
void kv_store_fill_header(uint8_t h[DS4_KVSTORE_FIXED_HEADER],
    ...
    h[6] = ext_flags;
    h[7] = model_id;
    kv_store_le_put32(h + 8, tokens);
    kv_store_le_put32(h + 12, hits);
    kv_store_le_put32(h + 16, ctx_size);
...
bool kv_store_read_header(FILE *fp, kv_store_entry *e,
    ...
    e->ext_flags = h[6];
    e->model_id = h[7];
    e->tokens = kv_store_le_get32(h + 8);
    e->hits = kv_store_le_get32(h + 12);
    e->ctx_size = kv_store_le_get32(h + 16);
    ...
    *text_bytes = kv_store_le_get32(tb);
...
bool kv_store_read_entry_file(const char *path, const char sha[41],
    ...
    bool ok = kv_store_read_header(fp, &e, &text_bytes);
    fclose(fp);
    if (!ok) return false;
...
static void kv_cache_refresh(kv_store *kc) {
    ...
    while ((de = readdir(d)) != NULL) {
        char sha[41];
        if (!kv_store_sha_hex_name(de->d_name, sha)) continue;
        char *path = kv_store_path_join(kc->dir, de->d_name);
        kv_store_entry e = {0};
        if (kv_store_read_entry_file(path, sha, &e)) kv_cache_push(kc, e);
        free(path);
    }
    closedir(d);
}
...
bool kv_store_touch_file(const char *path, uint32_t hits) {
    ...
    if (ok) {
        ...
        kv_store_fill_header(h, e.model_id, e.quant_bits, e.reason, e.ext_flags,
...
static bool kv_cache_incoming_supersedes_continued(
    ...
    char prefix_sha[41];
    kv_store_sha1_bytes_hex(incoming->text, (size_t)e->text_bytes,
                               prefix_sha);
...
double kv_store_entry_eviction_score(
        const kv_store_entry *e,
...
void kv_store_evict(kv_store *kc, const ds4_tokens *live,
    ...
    while (total > target && kc->len > 0) {
        int victim = 0;
        double victim_score =
            kv_store_entry_eviction_score(&kc->entry[0], live, now,
                                             incoming);
        for (int i = 1; i < kc->len; i++) {
            double score =
                kv_store_entry_eviction_score(&kc->entry[i], live, now,
                                                 incoming);
        ...
        kv_store_entry e = kc->entry[victim];
        if (unlink(e.path) == 0) {
...
static int kv_cache_continued_step(const kv_store *kc) {
...
int kv_store_suppress_continued_store(kv_store *kc, int tokens) {
    if (kv_store_continued_store_target(kc, tokens) != tokens) return -1;
    int old = kc->continued_last_store_tokens;
    kv_store_note_store(kc, tokens);
    return old;
}
...
static bool kv_cache_file_text_matches(const char *path, const char sha[41],
    ...
    kv_store_entry hdr = {0};
    uint32_t text_bytes = 0;
    bool ok = kv_store_read_header(fp, &hdr, &text_bytes) &&
    ...
    char stored_sha[41];
    kv_store_sha1_bytes_hex(stored, text_bytes, stored_sha);
...
static bool kv_cache_existing_compatible(kv_store *kc, const char *path,
                                         const char sha[41],
                                         const char *text, size_t text_len,
                                         int model_id, int quant_bits, int ctx_size) {
    if (access(path, F_OK) != 0) return false;
    kv_store_entry e = {0};
    if (!kv_store_read_entry_file(path, sha, &e)) return false;
...
static bool kv_trailer_serialized_size(const kv_store_trailer_hooks *hooks,
                                       const char *text,
                                       uint64_t *bytes_out) {
...
static bool kv_trailer_write(const kv_store_trailer_hooks *hooks,
...
static void kv_cache_rewrite_trailer(kv_store *kc, const char *path,
                                     const char *text,
        ...
        ftruncate(fileno(fp), (off_t)end) == 0)
    {
        ...
        if (ok && ignored > 0) {
            uint8_t h[DS4_KVSTORE_FIXED_HEADER];
            uint64_t now = (uint64_t)time(NULL);
            kv_store_fill_header(h, hdr.model_id, hdr.quant_bits, hdr.reason,
...
bool kv_store_store_live_prefix_text(kv_store *kc,
    ...
    kv_store_tokens_copy_prefix(&store_tokens, tokens, store_len);
    ...
    if (text_override) {
        ...
        text = kv_store_render_tokens_text(engine, &store_tokens, &text_len);
    }
    ...
    char sha[41];
    kv_store_sha1_bytes_hex(text, text_len, sha);
    char *path = kv_store_path_for_sha(kc, sha);
    const uint8_t reason_code = kv_store_reason_code(reason);
    ...
    if (!kv_store_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                    trailer_est_bytes,
    ...
    kv_store_evict(kc, live_tokens, est_file_bytes, &incoming);
    ...
    if (ok && !kv_store_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                          trailer_bytes,
                                          &final_file_bytes,
...
bool kv_store_store_live_prefix(kv_store *kc,
                                   ...
                                   char *err,
                                   size_t err_len) {
    return kv_store_store_live_prefix_text(kc, engine, session, tokens,
...
bool kv_store_maybe_store_continued(kv_store *kc,
    ...
    const int target = kv_store_continued_store_target(kc, tokens->len);
    if (target == 0) return false;
    if (kv_store_store_live_prefix(kc, engine, session, tokens, target,
                                      "continued", hooks, err, err_len))
    {
        kv_store_note_store(kc, target);
        return true;
    }
    return false;
}
...
int kv_store_try_load_text(kv_store *kc,
    ...
    const int model_id = ds4_engine_model_id(engine);
    const size_t prompt_bytes = strlen(prompt_text);
    int idx = kv_store_find_text_prefix(kc, prompt_text, model_id, quant_bits,
                                           ds4_session_ctx(session));
    if (idx < 0) return 0;

    kv_store_entry e = kc->entry[idx];
    ...
    bool header_ok = kv_store_read_header(fp, &hdr, &text_bytes);
    ...
    if (header_ok) {
        if (hdr.model_id != (uint8_t)model_id) {
            ...
            if (fread(cached_text, 1, text_bytes, fp) != text_bytes) {
                ...
                char text_sha[41];
                kv_store_sha1_bytes_hex(cached_text, text_bytes, text_sha);
                if (strcmp(text_sha, e.sha)) {
                    header_ok = false;
                    fail_reason = "cached text hash mismatch";
                } else if (!kv_store_byte_prefix_match(prompt_text, prompt_bytes,
    ...
    if (loaded > 0) {
        ...
        const char *key_kind = kv_store_key_kind(hdr.ext_flags);
        kv_store_touch_file(path, hdr.hits + 1);
        kv_logf(kc, DS4_KVSTORE_LOG_KVCACHE,
