...
static const char *tool_name(cli_tool tool) {
    switch (tool) {
    case DS4_HELP_DS4: return "ds4";
    case DS4_HELP_SERVER: return "glm-server";
    case DS4_HELP_AGENT: return "ds4-agent";
    case DS4_HELP_BENCH: return "glm-bench";
    case DS4_HELP_EVAL: return "ds4-eval";
    }
    return "ds4";
}

static const char *tool_usage(cli_tool tool) {
    switch (tool) {
    case DS4_HELP_DS4:
        return "Usage: ds4 [(-p PROMPT | --prompt-file FILE)] [options]";
    case DS4_HELP_SERVER:
        return "Usage: glm-server [options]";
    case DS4_HELP_AGENT:
        return "Usage: ds4-agent [options]";
    case DS4_HELP_BENCH:
        return "Usage: glm-bench (--prompt-file FILE | --chat-prompt-file FILE) [options]";
    case DS4_HELP_EVAL:
...
static const char *tool_summary(cli_tool tool) {
    switch (tool) {
    case DS4_HELP_DS4:
...
static bool tool_has_topic(cli_tool tool, const char *topic) {
...
static void print_more_info(FILE *fp, const help_colors *c, cli_tool tool) {
...
static void print_examples(FILE *fp, const help_colors *c, cli_tool tool, const char *topic) {
    title(fp, c, "Examples");
    if (topic_is(topic, "distributed")) {
        ...
        if (tool == DS4_HELP_SERVER) {
            opt(fp, c, "Metal API", "./glm-server -m ds4flash.gguf --metal --ctx 100000");
            opt(fp, c, "quiet API", "./glm-server --power 60 --host 127.0.0.1 --port 8000");
        } else if (tool == DS4_HELP_AGENT) {
            opt(fp, c, "agent", "./ds4-agent -m ds4flash.gguf --ctx 100000");
            opt(fp, c, "quiet agent", "./ds4-agent --power 50");
        } else if (tool == DS4_HELP_BENCH) {
            opt(fp, c, "bench", "./glm-bench --prompt-file long.txt --ctx-max 32768");
            opt(fp, c, "quiet bench", "./glm-bench --prompt-file long.txt --power 70");
        } else if (tool == DS4_HELP_EVAL) {
        ...
        opt(fp, c, "local API", "./glm-server --ctx 100000 --kv-disk-dir ~/.ds4/server-kv --kv-disk-space-mb 8192");
        ...
        opt(fp, c, "csv", "./glm-bench --prompt-file long.txt --ctx-max 32768 --csv speed.csv");
        opt(fp, c, "prefill only", "./glm-bench --prompt-file long.txt --gen-tokens 0");
...
static void print_topic(FILE *fp, const help_colors *c, cli_tool tool, const char *topic) {
    if (streq(topic, "all")) {
...
static void print_default(FILE *fp, const help_colors *c, cli_tool tool) {
