...
static uint64_t bench_snapshot_max_bytes(void) {
    ...
    if (env[0] == '\0' || !end || *end != '\0') {
        fprintf(stderr,
                "glm-bench: invalid DS4_BENCH_SNAPSHOT_MAX_BYTES=%s; using default %llu\n",
                env,
...
static void usage(FILE *fp, const char *topic) {
    cli_print(fp, DS4_HELP_BENCH, topic);
}
...
static int parse_int(const char *s, const char *opt) {
    ...
    if (s[0] == '\0' || *end != '\0' || v <= 0 || v > INT_MAX) {
        fprintf(stderr, "glm-bench: invalid value for %s: %s\n", opt, s);
        exit(2);
    }
    return (int)v;
}
...
static const char *need_arg(int *i, int argc, char **argv, const char *opt) {
    if (*i + 1 >= argc) {
        fprintf(stderr, "glm-bench: %s requires an argument\n", opt);
        exit(2);
    }
    return argv[++*i];
}
...
static ds4_backend parse_backend(const char *s, const char *opt) {
    ...
    fprintf(stderr, "glm-bench: invalid value for %s: %s\n", opt, s);
#ifdef DS4_ROCM_BUILD
    fprintf(stderr, "glm-bench: valid backends are: metal, rocm, cpu\n");
#else
    fprintf(stderr, "glm-bench: valid backends are: metal, cuda, cpu\n");
#endif
    exit(2);
}
...
static char *read_file(const char *path) {
    ...
    if (!fp) {
        fprintf(stderr, "glm-bench: failed to open %s: %s\n", path, strerror(errno));
        exit(1);
    }
    if (fseek(fp, 0, SEEK_END) != 0) {
        fprintf(stderr, "glm-bench: failed to seek %s\n", path);
        fclose(fp);
        exit(1);
    }
    long n = ftell(fp);
    if (n < 0) {
        fprintf(stderr, "glm-bench: failed to tell %s\n", path);
        fclose(fp);
        exit(1);
    }
    if (fseek(fp, 0, SEEK_SET) != 0) {
        fprintf(stderr, "glm-bench: failed to rewind %s\n", path);
        fclose(fp);
        exit(1);
    }
    char *buf = malloc((size_t)n + 1);
    if (!buf) {
        fprintf(stderr, "glm-bench: out of memory reading %s\n", path);
        fclose(fp);
        exit(1);
    }
    if (fread(buf, 1, (size_t)n, fp) != (size_t)n) {
        fprintf(stderr, "glm-bench: failed to read %s\n", path);
        free(buf);
        fclose(fp);
        exit(1);
    }
...
static bench_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        tensor_parallel_cli_parse_result tp_parse =
            tensor_parallel_parse_cli_arg(arg,
                                 &i,
                                 argc,
                                 argv,
                                 &c.tp,
        ...
        if (!strcmp(arg, "-m") || !strcmp(arg, "--model")) {
            ...
            if (!ds4_parse_streaming_cache_experts_arg(
                ...
                fprintf(stderr,
                        "glm-bench: --ssd-streaming-cache-experts must be a positive count or <number>GB\n");
                exit(2);
            }
            ...
            if (v <= 0) {
                fprintf(stderr, "glm-bench: --ssd-streaming-preload-experts must be positive\n");
                exit(2);
            }
            ...
            if (!ds4_parse_gib_arg(need_arg(&i, argc, argv, arg),
                ...
                fprintf(stderr,
                        "glm-bench: --simulate-used-memory must be a positive GiB value, e.g. 64GB\n");
                exit(2);
            }
            ...
            if (c.power_percent < 1 || c.power_percent > 100) {
                fprintf(stderr, "glm-bench: --power must be between 1 and 100\n");
                exit(2);
            }
            ...
            fprintf(stderr, "glm-bench: unknown option: %s\n", arg);
            usage(stderr, NULL);
            exit(2);
        }
    }
    ...
    if (!!c.prompt_path == !!c.chat_prompt_path) {
        fprintf(stderr, "glm-bench: specify exactly one of --prompt-file or --chat-prompt-file\n");
        exit(2);
    }
    if (c.ctx_start > c.ctx_max) {
        fprintf(stderr, "glm-bench: --ctx-start must be <= --ctx-max\n");
        exit(2);
    }
    if (c.step_mul < 1.0) {
        fprintf(stderr, "glm-bench: --step-mul must be >= 1\n");
        exit(2);
    }
    if (c.step_mul == 1.0 && c.step_incr <= 0) {
        fprintf(stderr, "glm-bench: --step-incr must be positive when --step-mul is 1\n");
        exit(2);
    }
    if (c.ctx_max > INT_MAX - c.gen_tokens - 1) {
        fprintf(stderr, "glm-bench: requested context is too large\n");
        exit(2);
    }
    if (c.ctx_alloc == 0) c.ctx_alloc = c.ctx_max + c.gen_tokens + 1;
    if (c.ctx_alloc <= c.ctx_max + c.gen_tokens) {
        fprintf(stderr, "glm-bench: --ctx-alloc must be greater than ctx-max + gen-tokens\n");
        exit(2);
    }
    char tp_err[256];
    if (!tensor_parallel_adopt_distributed_options(&c.tp,
                                          &c.dist,
                                          tp_err,
                                          sizeof(tp_err))) {
        fprintf(stderr, "glm-bench: %s\n", tp_err);
        exit(2);
    }
    char dist_err[256];
    ...
    if (c.dist.role == DS4_DISTRIBUTED_WORKER || c.tp.role == DS4_TP_WORKER) {
        fprintf(stderr, "glm-bench: --role worker is a serving mode; start workers with ./ds4\n");
        exit(2);
    }
    return c;
}
...
static int write_frontier_logits_json(
    ...
    if (!logits) {
        fprintf(stderr, "glm-bench: out of memory copying frontier logits\n");
        return 1;
    }
    if (ds4_session_copy_logits(session, logits, vocab) != vocab) {
        fprintf(stderr, "glm-bench: failed to copy frontier logits at %d\n", frontier);
        free(logits);
        return 1;
    }
    ...
    if (n <= 0 || (size_t)n >= sizeof(path)) {
        fprintf(stderr, "glm-bench: frontier logits path is too long\n");
        free(logits);
        return 1;
    }
    ...
    fputs("\n  ]\n}\n", fp);
    if (fclose(fp) != 0) {
        fprintf(stderr, "glm-bench: failed to close %s\n", path);
        free(logits);
        return 1;
    }
    free(logits);
...
static void log_context_memory(ds4_backend backend,
    ...
    ds4_context_memory m =
        ds4_context_memory_estimate_with_prefill_mode(backend,
                                                      ...
                                                      ssd_streaming);
    fprintf(stderr,
            "glm-bench: context buffers %.2f MiB (ctx=%d, backend=%s, prefill_chunk=%u, raw_kv_rows=%u, compressed_kv_rows=%u)\n",
...
static int wait_distributed_route(ds4_session *session) {
    ...
    for (;;) {
        ...
        if (ready > 0) {
            if (ticks) fprintf(stderr, "glm-bench: distributed route ready\n");
            return 0;
        }
        if (ready < 0) {
            fprintf(stderr,
                    "glm-bench: distributed route readiness failed: %s\n",
                    err[0] ? err : "unknown error");
            return 1;
        }
        const char *why = err[0] ? err : "route incomplete";
        if (strcmp(last, why) != 0 || (ticks % 20u) == 0) {
            fprintf(stderr, "glm-bench: waiting for distributed route: %s\n", why);
...
static void maybe_warn_distributed_step_shape(const bench_config *cfg, ds4_session *session) {
    ...
    {
        fprintf(stderr,
                "glm-bench: note: --step-incr=%d is smaller than distributed prefill chunk %u; "
...
static void close_engine(ds4_engine *engine, tensor_parallel *tp) {
    if (tp) tensor_parallel_send_stop(tp);
...
int main(int argc, char **argv) {
    ...
    char tp_err[256];
    if (!tensor_parallel_validate_engine_options(&opt, tp_err, sizeof(tp_err))) {
        fprintf(stderr, "glm-bench: %s\n", tp_err);
        return 2;
    }
    ...
    tensor_parallel *tp_leader = NULL;
    if (cfg.tp.role == DS4_TP_LEADER) {
        tensor_parallel_identity tp_id = {
        ...
        if (!tensor_parallel_create(&tp_leader,
                           &cfg.tp,
                           &tp_id,
                           tp_err,
                           sizeof(tp_err)) ||
    ...
    if (prompt.len < prompt_tokens_needed) {
        fprintf(stderr,
                "glm-bench: prompt has %d tokens, need at least %d%s\n",
                prompt.len,
    ...
    if (ds4_session_create(&session, engine, cfg.ctx_alloc) != 0) {
        fprintf(stderr, "glm-bench: failed to create session\n");
        ds4_tokens_free(&prompt);
        close_engine(engine, tp_leader);
        return 1;
    }
    ...
    for (int frontier = cfg.ctx_start; ; frontier = next_frontier(&cfg, frontier)) {
...
#endif
        if (prefill_rc != 0) {
            fprintf(stderr, "glm-bench: prefill to %d failed: %s\n", frontier, err);
            rc = 1;
            break;
        }
        ...
        if (need_restore_after_generation && !distributed &&
            ...
            if (payload_bytes > snapshot_max_bytes && !large_snapshot_forced) {
                if (!warned_large_snapshot) {
                    fprintf(stderr,
                            "glm-bench: session payload snapshot is %.2f GiB, above the %.2f GiB benchmark limit; "
                ...
                if (ds4_session_save_snapshot(session, &snap, err, sizeof(err)) != 0) {
                    fprintf(stderr, "glm-bench: snapshot at %d failed: %s\n", frontier, err);
                    rc = 1;
                    break;
                }
                have_snapshot = true;
        ...
        if (cuda_profile_range &&
            ...
            fprintf(stderr,
                    "glm-bench: invalid DS4_BENCH_CUDA_PROFILE_RANGE=%s "
        ...
        for (int i = 0; i < cfg.gen_tokens; i++) {
            if (ds4_session_pos(session) + 1 >= ds4_session_ctx(session)) {
                fprintf(stderr, "glm-bench: generation would exceed allocated context at frontier %d\n", frontier);
                rc = 1;
                break;
            }
            ...
            if (token < 0) {
                fprintf(stderr, "glm-bench: failed to choose non-EOS token at frontier %d\n", frontier);
                rc = 1;
                break;
            }
            ...
            if (ds4_session_eval(session, token, err, sizeof(err)) != 0) {
                fprintf(stderr, "glm-bench: decode at frontier %d failed: %s\n", frontier, err);
                rc = 1;
                break;
            }
#if defined(DS4_BENCH_HAVE_CUDA_PROFILER)
        ...
        if (cfg.show_output && gen_token_buf && gen_token_count > 0) {
            fprintf(stderr, "glm-bench: gen[ctx=%d] decoded text: \"", frontier);
        ...
        if (!need_restore_after_generation) {
            ...
            if (ds4_session_sync(session, &prefix, err, sizeof(err)) != 0) {
                fprintf(stderr, "glm-bench: replay restore at %d failed: %s\n", frontier, err);
                rc = 1;
                break;
            }
        } else {
            if (ds4_session_load_snapshot(session, &snap, err, sizeof(err)) != 0) {
                fprintf(stderr, "glm-bench: restore at %d failed: %s\n", frontier, err);
                rc = 1;
                break;
            }
        }
