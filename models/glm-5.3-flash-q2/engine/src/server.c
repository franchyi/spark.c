    ...
    if (require_thinking_closed) {
        ...
        if (!think_end) {
            ...
            if (!candidate || !find_any_tool_end(candidate)) {
                fprintf(stderr, "glm-server: thinking not closed, ignoring incomplete DSML in reasoning\n");
    ...
    if (require_thinking_closed) {
        ...
        if (!think_end) {
            ...
            if (!candidate || !strstr(candidate, tool_end)) {
                fprintf(stderr, "glm-server: thinking not closed, ignoring incomplete GLM tool calls in reasoning\n");
                ds4_local_unterminated_reasoning(text, content_out, reasoning_out);
                return true;
            }
            tool_search = candidate;
...
typedef kv_store_entry kv_entry;
typedef kv_store_options kv_cache_options;
typedef kv_store kv_disk_cache;
...
static bool sha_hex_name(const char *name, char sha[41]) {
    return kv_store_sha_hex_name(name, sha);
}

static char *path_join(const char *dir, const char *name) {
    return kv_store_path_join(dir, name);
}
...
static double kv_entry_eviction_score(const kv_entry *e, const ds4_tokens *live,
    ...
    return kv_store_entry_eviction_score(e, live, now, incoming);
}
#endif
...
static void kv_cache_evict(kv_disk_cache *kc, const ds4_tokens *live,
                           ...
                           const kv_store_eviction_context *incoming) {
    kv_store_evict(kc, live, extra_bytes, incoming);
}
#endif

static void kv_cache_log_cb(void *ud, kv_store_log_type type, const char *msg) {
    (void)ud;
...
static bool kv_cache_open(kv_disk_cache *kc, const char *dir, uint64_t budget_mb,
    ...
    return kv_store_open(kc, dir, budget_mb, reject_different_quant, opt,
                            "glm-server", kv_cache_log_cb, NULL);
}
...
static char *render_tokens_text(ds4_engine *engine, const ds4_tokens *tokens, size_t *out_len) {
    return kv_store_render_tokens_text(engine, tokens, out_len);
}

static bool byte_prefix_match(const char *text, size_t text_len,
                              const char *prefix, size_t prefix_len) {
    return kv_store_byte_prefix_match(text, text_len, prefix, prefix_len);
}


static void tokens_copy_prefix(ds4_tokens *dst, const ds4_tokens *src, int n) {
    kv_store_tokens_copy_prefix(dst, src, n);
}
...
static int kv_cache_chat_anchor_pos(const kv_disk_cache *kc,
    ...
    return kv_store_chat_anchor_pos(kc, prompt, user_token_id, assistant_token_id);
}


static int kv_cache_continued_store_target(const kv_disk_cache *kc, int live_tokens) {
    return kv_store_continued_store_target(kc, live_tokens);
}
...
static bool kv_cache_file_size_fits(const kv_disk_cache *kc,
    ...
    return kv_store_file_size_fits(kc, text_bytes, payload_bytes,
...
static kv_store_trailer_hooks kv_cache_tool_map_hooks(server *s,
                                                         const stop_list *wanted) {
    return (kv_store_trailer_hooks){
        .ud = s,
...
static bool kv_cache_store_live_prefix_text(server *s, server_slot *slot,
    ...
    char err[160] = {0};
    kv_store_trailer_hooks hooks = kv_cache_tool_map_hooks(s, NULL);
    pthread_mutex_lock(&s->inference_mu);
    pthread_mutex_lock(&s->kv_mu);
    bool ok = kv_store_store_live_prefix_text(&s->kv, s->engine,
                                                  slot->session,
...
static int kv_cache_suppress_continued_store(kv_disk_cache *kc, int tokens) {
    return kv_store_suppress_continued_store(kc, tokens);
}

static void kv_cache_restore_suppressed_continued(kv_disk_cache *kc,
                                                  int old_tokens,
                                                  int suppressed_tokens) {
    kv_store_restore_suppressed_continued(kc, old_tokens, suppressed_tokens);
}
#endif
...
static void kv_cache_discard_failed_disk_entry(server *s, server_slot *slot,
    ...
    if (unlink(path) == 0) {
        server_log(DS4_LOG_KVCACHE,
                   "glm-server: kv cache discarded reason=prefill-failed file=%s",
                   path);
    } else if (errno != ENOENT) {
        server_log(DS4_LOG_WARNING,
                   "glm-server: kv cache failed to discard prefill-failed file=%s: %s",
                   path, strerror(errno));
    }
    pthread_mutex_unlock(&s->kv_mu);
...
static int kv_cache_find_text_prefix(kv_disk_cache *kc, const char *prompt_text,
    ...
    return kv_store_find_text_prefix(kc, prompt_text, 0, quant_bits, ctx_size);
}
#endif
...
static int kv_cache_try_load_text(server *s, server_slot *slot,
    ...
    kv_store_load_result lr = {0};
    kv_store_trailer_hooks hooks = kv_cache_tool_map_hooks(s, NULL);
    pthread_mutex_lock(&s->inference_mu);
    pthread_mutex_lock(&s->kv_mu);
    int loaded = kv_store_try_load_text(&s->kv, s->engine, slot->session,
...
static void log_decode_progress(req_kind kind, int prompt_tokens, int completion,
    ...
    server_log(DS4_LOG_GENERATION,
               "glm-server: %s ctx=%s gen=%d%s%s decoding chunk=%.2f t/s avg=%.2f t/s %.3fs",
...
static void log_tool_calls_summary(const char *ctx, const tool_calls *calls,
    ...
    server_log(DS4_LOG_TOOL,
               "glm-server: tool calls ctx=%s%s%s n=%d raw_tool_text=%d ids=[%s] names=[%s]",
               ctx,
               flags[0] ? " " : "",
               flags,
               calls->len,
...
static void server_progress_cb(void *ud, const char *event, int current, int total) {
    ...
    server_log(DS4_LOG_PREFILL,
               "glm-server: %s ctx=%s%s%s %s chunk %d/%d (%.1f%%) chunk=%.2f t/s avg=%.2f t/s %.3fs",
...
static void send_prefill_failure_response(server *s, const job *j,
    ...
    if (j->req.stream && progress && progress->headers_sent) {
        if (progress->stream_failed) {
            server_log(DS4_LOG_GENERATION,
                       "glm-server: %s ctx=%s%s%s prefill failed after stream closed: %s",
        ...
        if (!sse_error_event(j->fd, &j->req, err)) {
            server_log(DS4_LOG_GENERATION,
                       "glm-server: %s ctx=%s%s%s prefill SSE error failed: %s",
                       kind, ctx, flags && flags[0] ? " " : "",
                       flags && flags[0] ? flags : "", err);
        }
        return;
...
static void remember_thinking_checkpoint(server *s, server_slot *slot,
    ...
    server_log(DS4_LOG_KVCACHE,
               "glm-server: thinking live checkpoint remembered ctx=%s live=%d visible=%zu",
...
static void canonicalize_tool_checkpoint(server *s, server_slot *slot,
    ...
    if (rr == DS4_SESSION_REWRITE_OK) {
        server_log(DS4_LOG_KVCACHE,
                   "glm-server: tool checkpoint canonicalized ctx=%s common=%d live=%d canonical=%d",
        ...
        server_log(DS4_LOG_KVCACHE,
                   "glm-server: tool checkpoint canonicalization needs %d tokens rebuild ctx=%s request_ctx=%s reason=canonical-tail-rewrite tail=%d discard=%d common=%d live=%d target=%d cached=%d source=%s%s%s",
                   replay_tokens,
                   rebuild_ctx,
                   ctx,
        ...
        if (server_session_sync(s, slot, sync_prompt,
            ...
            if (loaded > 0) {
                server_log(DS4_LOG_KVCACHE,
                           "glm-server: tool checkpoint rebuild done ctx=%s request_ctx=%s source=disk cached=%d replay=%d target=%d %.3fs",
                ...
                server_log(DS4_LOG_KVCACHE,
                           "glm-server: tool checkpoint rebuild done ctx=%s request_ctx=%s source=full cached=0 replay=%d target=%d %.3fs",
            ...
            ds4_session_set_display_progress(slot->session, NULL, NULL);
            server_log(DS4_LOG_KVCACHE,
                       "glm-server: tool checkpoint rebuild failed ctx=%s request_ctx=%s source=%s cached=%d replay=%d target=%d error=\"%s\"",
        ...
        server_log(DS4_LOG_KVCACHE,
                   "glm-server: tool checkpoint canonicalization failed ctx=%s common=%d live=%d canonical=%d error=\"%s\"",
    ...
    for (;;) {
        ...
        if (log_batches) {
            server_log(DS4_LOG_DEFAULT,
                       "glm-server: decode batch count=%d elapsed=%.3f ms status=%s",
...
static void generate_job_inner(server *s, server_slot *slot, job *j) {
               ...
               j->req.anthropic_requires_live_tool_state)
    {
        ...
        if (rewind_to >= 0) {
            ...
            server_log(DS4_LOG_KVCACHE,
                       "glm-server: rewound GLM live prefix from %d to %d; final prompt token will be reevaluated",
                       old_pos, rewind_to);
        } else {
    ...
    if (cached == 0 && old_pos > 0) {
        server_log(DS4_LOG_WARNING,
                   "glm-server: live kv cache miss%s live=%d prompt=%d common=%d reason=%s",
    ...
    if (responses_live_continuation) {
        server_log(DS4_LOG_PREFILL,
                   "glm-server: responses live continuation RESPPROTO match=%s ids=%d cached=%d prompt=%d",
        ...
        server_log(DS4_LOG_PREFILL,
                   "glm-server: anthropic live continuation match=tool-output-ids ids=%d cached=%d prompt=%d",
                   anthropic_live_match_ids,
                   cached,
                   prompt_tokens);
    } else if (thinking_live_continuation) {
        server_log(DS4_LOG_PREFILL,
                   "glm-server: thinking live continuation match=visible-prefix cached=%d prompt=%d",
                   cached,
                   prompt_tokens);
    }
    if (responses_visible_replay_without_reasoning) {
        ...
        server_log(DS4_LOG_WARNING,
                   "glm-server: responses replay RESPPROTO missing reasoning state; continuing from visible history source=%s cached=%d prompt=%d",
                   cache_source,
                   cached,
                   prompt_tokens);
        trace_event(s, trace_id,
    ...
    server_log(DS4_LOG_PREFILL,
               "glm-server: %s ctx=%s%s%s prompt start",
               j->req.kind == REQ_CHAT ? "chat" : "completion",
               ctx_span,
               req_flags[0] ? " " : "",
               req_flags);
    ...
    server_log(DS4_LOG_PREFILL,
               "glm-server: %s ctx=%s%s%s prompt done %.3fs",
               j->req.kind == REQ_CHAT ? "chat" : "completion",
               ctx_span,
               req_flags[0] ? " " : "",
               req_flags,
    ...
    if (j->req.stream) {
        if (progress.stream_failed) {
            server_log(DS4_LOG_GENERATION,
                       "glm-server: %s ctx=%s%s%s stream closed during prefill",
                       j->req.kind == REQ_CHAT ? "chat" : "completion",
                       ctx_span,
                       req_flags[0] ? " " : "",
                       req_flags);
        ...
        if (!progress.headers_sent && !sse_headers(j->fd, s->enable_cors)) {
            ...
            server_log(DS4_LOG_GENERATION,
                       "glm-server: %s ctx=%s%s%s sse headers failed",
                       j->req.kind == REQ_CHAT ? "chat" : "completion",
                       ctx_span,
                       req_flags[0] ? " " : "",
                       req_flags);
        ...
        if (j->req.api == API_ANTHROPIC &&
            ...
            job_mark_cancelled(j);
            server_log(DS4_LOG_GENERATION, "glm-server: chat ctx=%s anthropic stream start failed", ctx_span);
            request_live_state_clear(s, slot);
            ds4_tokens_free(&effective_prompt);
            return;
        }
        if (j->req.api == API_OPENAI && j->req.kind == REQ_CHAT &&
            ...
            job_mark_cancelled(j);
            server_log(DS4_LOG_GENERATION, "glm-server: chat ctx=%s openai role chunk failed", ctx_span);
            request_live_state_clear(s, slot);
            ds4_tokens_free(&effective_prompt);
            return;
        }
        ...
        if (responses_live_chat) {
            ...
            if (!responses_sse_created(j->fd, &j->req, &responses_live, responses_created_at)) {
                job_mark_cancelled(j);
                server_log(DS4_LOG_GENERATION,
                           "glm-server: chat ctx=%s%s%s responses created event failed",
                           ctx_span,
                           req_flags[0] ? " " : "",
                           req_flags);
...
decode_again:
            ...
            if (j->req.kind == REQ_CHAT && j->req.has_tools) {
                if (thinking_gates_tool_markers && thinking.inside) {
                    ...
                    if (complete_tool_call_inside_thinking(
                        ...
                        stop_decode = true;
                        server_log(DS4_LOG_WARNING,
                                   "glm-server: chat ctx=%s%s%s recovered a complete tool call from unclosed reasoning after %d generated tokens",
                                   ctx_span,
                                   req_flags[0] ? " " : "",
                                   req_flags,
                                   completion);
                    ...
                    if (orphan_end && !saw_orphan_tool_end) {
                        ...
                        server_log(DS4_LOG_WARNING,
                                   "glm-server: chat ctx=%s%s%s ignored orphan tool-call end marker after %d generated tokens",
                                   ctx_span,
                                   req_flags[0] ? " " : "",
                                   req_flags,
                                   completion);
        ...
        saw_tool_start && !saw_tool_end && strcmp(finish, "error") != 0)
    {
        ...
        if (try_repair_dsml(text.ptr, text.len, &repaired)) {
            ...
            if (repair_ok && test_calls.len > 0) {
                ...
                server_log(DS4_LOG_WARNING,
                           "glm-server: chat ctx=%s%s%s repaired unterminated tool call (%d calls recovered)",
                           ctx_span,
                           req_flags[0] ? " " : "",
                           req_flags,
                           test_calls.len);
        ...
        if (!completed_truncation) {
            if (!j->req.stream && !dsml_recovery_attempted) {
                ...
                server_log(DS4_LOG_WARNING,
                           "glm-server: chat ctx=%s%s%s unterminated tool call; continuing with model-visible tool error",
                           ctx_span,
                           req_flags[0] ? " " : "",
                           req_flags);
                trace_event(s, trace_id,
                                                ...
                                                sizeof(recovery_err)))
                {
                    ...
                    server_log(DS4_LOG_GENERATION,
                               "glm-server: chat ctx=%s%s%s tool-error continuation appended %d tokens",
                               ctx_span,
                               req_flags[0] ? " " : "",
                               req_flags,
                               recovery_tokens);
    ...
    if (j->req.kind == REQ_CHAT) {
        ...
        if (!parsed_ok && recovered_tool_parse_failure && j->req.has_tools && saw_tool_start) {
            ...
            if (!j->req.stream && !dsml_recovery_attempted) {
                ...
                server_log(DS4_LOG_WARNING,
                           "glm-server: chat ctx=%s%s%s invalid tool call; continuing with model-visible tool error",
                           ctx_span,
                           req_flags[0] ? " " : "",
                           req_flags);
                trace_event(s, trace_id,
            ...
            if (!parsed_ok) {
                ...
                server_log(DS4_LOG_WARNING,
                           "glm-server: chat ctx=%s%s%s invalid tool call returned as assistant text finish=%s [text_len=%zu saw_start=%d saw_end=%d text_snippet: %.*s]",
                           ctx_span,
                           req_flags[0] ? " " : "",
                           req_flags,
                           final_finish,
                ...
                server_log(DS4_LOG_WARNING,
                           "glm-server: chat ctx=%s%s%s invalid tool call dsml_snippet: %.*s",
                           ctx_span,
                           req_flags[0] ? " " : "",
                           req_flags,
    ...
    if (!response_ok) {
        ...
        request_live_state_clear(s, slot);
        server_log(DS4_LOG_DEFAULT,
                   "glm-server: %s ctx=%s%s%s client disconnected",
                   j->req.kind == REQ_CHAT ? "chat" : "completion",
                   ctx_span,
                   req_flags[0] ? " " : "",
                   req_flags);
    ...
    if (j->req.kind == REQ_CHAT && j->req.has_tools) {
        ...
        if (!strcmp(final_finish, "error") && err[0]) {
            server_log(DS4_LOG_GENERATION,
                       "glm-server: chat ctx=%s gen=%d%s%s finish=%s error=\"%s\" %.3fs",
                       ctx_span,
                       completion,
                       flags[0] ? " " : "",
                       flags,
            ...
            server_log(DS4_LOG_GENERATION,
                       "glm-server: chat ctx=%s gen=%d%s%s finish=%s %.3fs",
                       ctx_span,
                       completion,
                       flags[0] ? " " : "",
                       flags,
        ...
        if (!strcmp(final_finish, "error") && err[0]) {
            server_log(DS4_LOG_GENERATION,
                       "glm-server: %s ctx=%s gen=%d%s%s finish=%s error=\"%s\" %.3fs",
                       j->req.kind == REQ_CHAT ? "chat" : "completion",
                       ctx_span,
                       completion,
                       flags[0] ? " " : "",
            ...
            server_log(DS4_LOG_GENERATION,
                       "glm-server: %s ctx=%s gen=%d%s%s finish=%s %.3fs",
                       j->req.kind == REQ_CHAT ? "chat" : "completion",
                       ctx_span,
                       completion,
                       flags[0] ? " " : "",
...
static int parse_int_arg(const char *s, const char *opt) {
    ...
    if (!s[0] || *end || v <= 0 || v > INT_MAX) {
        server_log(DS4_LOG_DEFAULT, "glm-server: invalid value for %s: %s", opt, s);
        exit(2);
    }
    return (int)v;
}
...
static const char *need_arg(int *i, int argc, char **argv, const char *opt) {
    if (*i + 1 >= argc) {
        server_log(DS4_LOG_DEFAULT, "glm-server: missing value for %s", opt);
        exit(2);
    }
    return argv[++(*i)];
}
...
static void log_context_memory(ds4_backend backend, int ctx_size,
    ...
    ds4_context_memory m =
        ds4_context_memory_estimate_with_prefill_mode(backend,
                                                      ...
                                                      ssd_streaming);
    server_log(DS4_LOG_DEFAULT,
               "glm-server: context buffers %.2f MiB (ctx=%d, backend=%s, prefill_chunk=%u, raw_kv_rows=%u, compressed_kv_rows=%u)",
    ...
    if (session_count > 1) {
        server_log(DS4_LOG_DEFAULT,
                   "glm-server: %d resident sessions request at least %.2f GiB of context buffers",
                   session_count,
...
static void usage(FILE *fp, const char *topic) {
    cli_print(fp, DS4_HELP_SERVER, topic);
}
...
static ds4_backend parse_backend_arg(const char *s, const char *arg) {
    ...
    server_log(DS4_LOG_DEFAULT, "glm-server: invalid %s value: %s", arg, s);
#ifdef DS4_ROCM_BUILD
    server_log(DS4_LOG_DEFAULT, "glm-server: valid server backends are: metal, rocm, cpu");
#else
    server_log(DS4_LOG_DEFAULT, "glm-server: valid server backends are: metal, cuda, cpu");
#endif
    exit(2);
}
...
static server_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "-m") || !strcmp(arg, "--model")) {
            ...
            if (!ds4_parse_streaming_cache_experts_arg(
                ...
                server_log(DS4_LOG_DEFAULT,
                           "glm-server: --ssd-streaming-cache-experts must be a positive count or <number>GB");
                exit(2);
            }
            ...
            if (v <= 0) {
                server_log(DS4_LOG_DEFAULT,
                           "glm-server: --ssd-streaming-preload-experts must be positive");
                exit(2);
            }
            ...
            if (!ds4_parse_gib_arg(need_arg(&i, argc, argv, arg),
                ...
                server_log(DS4_LOG_DEFAULT,
                           "glm-server: --simulate-used-memory must be a positive GiB value, e.g. 64GB");
                exit(2);
            }
            ...
            if (v <= 0) {
                server_log(DS4_LOG_DEFAULT,
                           "glm-server: --prefill-chunk must be positive");
                exit(2);
            }
            ...
            if (c.engine.power_percent < 1 || c.engine.power_percent > 100) {
                server_log(DS4_LOG_DEFAULT, "glm-server: --power must be between 1 and 100");
                exit(2);
            }
            ...
            server_log(DS4_LOG_DEFAULT, "glm-server: unknown option: %s", arg);
            usage(stderr, NULL);
            exit(2);
        }
    }
    ...
    {
        server_log(DS4_LOG_DEFAULT,
                   "glm-server: --kv-cache-cold-max-tokens must be 0 or >= --kv-cache-min-tokens");
        exit(2);
    }
    ...
    if (ds4_dist_prepare_engine_options(&c.engine.distributed,
                                        ...
                                        dist_err,
                                        sizeof(dist_err)) != 0) {
        server_log(DS4_LOG_DEFAULT, "glm-server: %s", dist_err);
        exit(2);
    }
    return c;
}
...
int main(int argc, char **argv) {
    ...
    if (cfg.chdir_path && chdir(cfg.chdir_path) != 0) {
        server_log(DS4_LOG_DEFAULT, "glm-server: failed to chdir to %s: %s",
    ...
    for (int i = 0; i < slot_count; i++) {
        ...
        if (ds4_session_create(&slot->session, engine, cfg.ctx_size) != 0) {
            server_log(DS4_LOG_DEFAULT,
                       "glm-server: failed to create %s session %d/%d",
    ...
    if (s.disable_exact_dsml_tool_replay) {
        server_log(DS4_LOG_DEFAULT,
                   "glm-server: exact DSML tool replay disabled; tool history uses canonical JSON rendering");
    }
    if (s.batched_mode) {
        server_log(DS4_LOG_DEFAULT,
                   "glm-server: batched mode enabled resident_sessions=%d prefill_quantum=%d mixed_prefill_quantum=%d decode_coalesce_us=%ld",
                   s.slot_count,
        ...
        if (ds4_engine_has_mtp(engine)) {
            server_log(DS4_LOG_DEFAULT,
                       "glm-server: MTP speculative decoding is disabled while native session batching is active");
        }
    }
    if (cfg.trace_path) {
        s.trace = fopen(cfg.trace_path, "w");
        if (!s.trace) {
            server_log(DS4_LOG_DEFAULT, "glm-server: failed to open trace file %s: %s",
                       cfg.trace_path, strerror(errno));
            server_close_resources(&s);
            return 1;
        }
        setvbuf(s.trace, NULL, _IONBF, 0);
        server_log(DS4_LOG_DEFAULT, "glm-server: tracing session to %s", cfg.trace_path);
    }
    ...
    if (s.batched_mode) {
        if (pthread_create(&s.decode_thread, NULL, decode_worker_main, &s) != 0) {
            server_log(DS4_LOG_DEFAULT, "glm-server: failed to start decode coordinator");
            server_close_resources(&s);
            return 1;
        }
        ...
        for (int i = 0; i < s.slot_count; i++) {
            if (pthread_create(&s.slot_threads[i], NULL, slot_worker_main,
                               &s.slots[i]) != 0) {
                server_log(DS4_LOG_DEFAULT,
                           "glm-server: failed to start session worker %d/%d",
                           i + 1, s.slot_count);
        ...
        server_log(DS4_LOG_DEFAULT, "glm-server: failed to start worker");
        server_close_resources(&s);
        return 1;
    }
    ...
    if (lfd < 0) {
        server_log(DS4_LOG_DEFAULT, "glm-server: failed to listen on %s:%d: %s", cfg.host, cfg.port, strerror(errno));
        server_request_worker_stop(&s);
        if (s.batched_mode) {
    ...
    g_listen_fd = lfd;
    server_log(DS4_LOG_DEFAULT, "glm-server: listening on http://%s:%d", cfg.host, cfg.port);
    ...
    while (!g_stop_requested) {
        ...
        if (fd < 0) {
            ...
            if (errno == EINTR) continue;
            server_log(DS4_LOG_DEFAULT, "glm-server: accept failed: %s", strerror(errno));
            continue;
        }
        if (g_stop_requested) {
            close(fd);
    ...
    server_log(DS4_LOG_DEFAULT, "glm-server: shutdown requested, draining requests");
    server_request_worker_stop(&s);
    if (s.batched_mode) {
        for (int i = 0; i < slot_threads_started; i++) {
            pthread_join(s.slot_threads[i], NULL);
    ...
    for (int i = 0; s.kv.enabled && i < s.slot_count; i++) {
        ...
        server_log(DS4_LOG_KVCACHE,
                   "glm-server: persisting resident KV cache before shutdown slot=%d tokens=%d",
                   i, tokens->len);
...
static void test_kv_text_stub_file_model(const char *dir, const char *text,
    ...
    uint8_t h[KV_CACHE_FIXED_HEADER];
    kv_store_fill_header(h, model_id, 2, reason, 0, tokens, 0,
...
static void test_kv_cache_lookup_rejects_wrong_model(void) {
    ...
    TEST_ASSERT(kv_store_find_text_prefix(&kc, "shared rendered prefix and tail",
                                             0, 2, 32768) < 0);
    int idx = kv_store_find_text_prefix(&kc, "shared rendered prefix and tail",
                                           1, 2, 32768);
    TEST_ASSERT(idx >= 0);
...
static void test_kv_cache_lookup_rejects_stale_payload_abi(void) {
    ...
    TEST_ASSERT(kv_store_find_text_prefix(&kc, "stale rendered prefix and tail",
                                             0, 2, 32768) < 0);
...
static void test_kv_cache_eviction_prefers_superseded_continued_prefix(void) {
    ...
    kv_store_eviction_context incoming = {
        .text = incoming_text,
...
int main(void) {
    ...
    if (test_failures) {
        fprintf(stderr, "glm-server tests: %d failure(s)\n", test_failures);
        return 1;
    }
    puts("glm-server tests: ok");
    return 0;
}
#endif
