/* repack: the aligned-SoA derived-weight layout library shared by the
 * weight server and the engine's in-process
 * self-load build (cuda/runtime.cu).  The GGUF tensor catalog walk, the repack
 ...
 * are shared with the consuming kernels in cuda/mmq/mmq_runtime.h. */
#ifndef DS4_REPACK_H
#define DS4_REPACK_H
 ...
 * manifest format and the engine's cuda_derived_kind (cuda/runtime.cu); the
 * layout comments live on the builders in repack.cu. */
enum {
...
struct repack_artifact {
    const repack_tensor *t = nullptr; /* borrowed from the caller's catalog */
 ...
 * cudaMalloc/cudaFree. */
typedef bool (*repack_alloc_fn)(void *ctx, repack_artifact *art);
typedef void (*repack_free_fn)(void *ctx, repack_artifact *art);

struct repack_build_args {
    ...
    const std::vector<repack_tensor> *records = nullptr;
    int device = 0;
    uint64_t copy_chunk_bytes = 0;   /* staged read chunk (weight server --copy-chunk-mb) */
    repack_alloc_fn alloc_fn = nullptr;
    repack_free_fn free_fn = nullptr;
