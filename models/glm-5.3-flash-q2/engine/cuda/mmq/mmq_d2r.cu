...
// mmq_d2r.cu - gated D2R Q2_K MoE down-GEMM production path.
//
...
bool mmq_runtime_q2_K_moe_d2r_available(int cc) {
...
bool mmq_runtime_iq2_xxs_moe_d2r_available(int cc) {
    static int cached_cc = -1;
    static int cached = 0;
    if (cached_cc != cc) {
        cached_cc = cc;
...
size_t mmq_runtime_q2_K_moe_d2r_scratch_bytes(int64_t ncols_max, int n_experts) {
...
size_t mmq_runtime_iq2_xxs_moe_d2r_pair_scratch_bytes(int64_t ncols_max, int n_experts) {
    const int64_t capacity = d2r_work_capacity(ncols_max, n_experts);
    if (capacity <= 0 || capacity > (int64_t)(INT_MAX - 1)) {
        return 0;
    }
...
size_t mmq_runtime_iq2_xxs_moe_d2r_fused_scratch_bytes(
...
int mmq_runtime_q2_K_moe_d2r_launch(const void *W_soa,
                                int64_t soa_blocks,
                                const void *q8,
                                ...
                                cudaStream_t stream) {
    const char *tag = "mmq_runtime_q2_K_moe_d2r_launch";
    const int dev = ggml_cuda_get_device();
    const int cc = ggml_cuda_info().devices[dev].cc;
    if (!mmq_runtime_q2_K_moe_d2r_available(cc)) {
        return 1;
    }
...
int mmq_runtime_iq2_xxs_moe_d2r_pair_launch(const void *gate_soa,
                                         const void *up_soa,
                                         int64_t soa_blocks,
                                         const void *q8,
                                         const int32_t *ids_dst,
                                         ...
                                         size_t worklist_scratch_bytes,
                                         cudaStream_t stream) {
    const char *tag = "mmq_runtime_iq2_xxs_moe_d2r_pair_launch";
    const int dev = ggml_cuda_get_device();
    const int cc = ggml_cuda_info().devices[dev].cc;
...
int mmq_runtime_iq2_xxs_moe_d2r_fused_launch(
        const void *gate_soa,
        const void *up_soa,
        int64_t soa_blocks,
        const void *input_q8,
        ...
        size_t worklist_scratch_bytes,
        cudaStream_t stream) {
    const char *tag = "mmq_runtime_iq2_xxs_moe_d2r_fused_launch";
    const int dev = ggml_cuda_get_device();
    const int cc = ggml_cuda_info().devices[dev].cc;
    if (!mmq_runtime_iq2_xxs_moe_d2r_available(cc)) {
        return 1;
...
// dispatch in cuda/runtime.cu keeps K=8192 on mmq.
// ============================================================================
...
bool mmq_runtime_q8_0_dense_d2r_available(int cc) {
    static int cached_cc = -1;
    static int cached = 0;
    if (cached_cc != cc) {
        cached_cc = cc;
...
int mmq_runtime_q8_0_dense_d2r_launch(
    ...
    if (!W_aligned || !q8 || !out ||
        ...
        fprintf(stderr, "mmq_runtime_q8_0_dense_d2r_launch: bad args M=%d N=%d K=%d\n", M, N, K);
        return -1;
    }
    ...
    if (!smem_opted) {
        ...
        if (aerr != cudaSuccess) {
            fprintf(stderr, "mmq_runtime_q8_0_dense_d2r_launch: smem opt-in failed: %s\n",
    ...
    if (err != cudaSuccess) {
        fprintf(stderr, "mmq_runtime_q8_0_dense_d2r_launch: launch failed: %s\n",
