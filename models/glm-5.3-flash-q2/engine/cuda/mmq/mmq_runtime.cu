...
// mmq_runtime.cu - host wrapper around llama.cpp's vendored mul_mat_q kernels.
//
// The fused target-prefill MoE dispatcher (mmq_runtime_fused_down,
// ds4_swiglu_weighted_f32, the fused_down branches of
// mmq_runtime_moe_pair_impl, and the mmq_runtime_iq2_xxs_q2_K_moe_fused_* entry
...
//
// Implements the public mmq_runtime_* entry points and explicitly instantiates
...
static bool mmq_runtime_nvtx_requested() {
...
static uint64_t mmq_runtime_nvtx_payload(uint32_t first, uint32_t second) {
    return ((uint64_t)first << 32) | second;
}

class mmq_runtime_nvtx_scope {
public:
    mmq_runtime_nvtx_scope(const char *name, uint64_t payload, bool enabled)
        : active_(enabled) {
#if DS4_MMQ_HAS_NVTX
        if (active_) {
    ...
    ~mmq_runtime_nvtx_scope() {
#if DS4_MMQ_HAS_NVTX
        if (active_) (void)nvtxRangePop();
#endif
    }

    mmq_runtime_nvtx_scope(const mmq_runtime_nvtx_scope &) = delete;
    mmq_runtime_nvtx_scope &operator=(const mmq_runtime_nvtx_scope &) = delete;
...
//
// Hypothesis: ggml_cuda_pool_alloc inside mmq_runtime_moe_vec_impl records a
...
// mmq_runtime_moe_vec_impl uses this persistent buffer instead of pool_alloc.
//
...
extern "C" void mmq_runtime_set_aligned_q81_scratch(void *ptr, size_t bytes) {
...
static void *mmq_runtime_aligned_q81_scratch(int device, size_t bytes) {
...
// Read by mmq_runtime_moe_vec_impl; non-zero means use the persistent buffer.
// Set by mmq_runtime_init once based on env.  (Single-threaded GPU work; no
// atomicity needed.)
extern "C" int mmq_runtime_q81_persistent_enabled(void) {
    return g_q81_scratch_enabled ? 1 : 0;
}

extern "C" void *mmq_runtime_q81_scratch_ptr(void) {
    return g_q81_scratch_ptr;
}

// M2-Inc2a: registry of producer-emitted q8_1 activations (cuda/runtime.cu).
...
extern "C" int ds4_cuda_q8_fold_take_q81(const void *src, uint64_t in_dim,
                                         const void **q81);
static char *mmq_runtime_folded_q81(const float *X_f32, int64_t K, int n_tokens,
                                int64_t ne10_padded) {
...
extern "C" size_t mmq_runtime_q81_scratch_bytes(void) {
    return g_q81_scratch_bytes;
}

extern "C" int mmq_runtime_init(int device) {
    if (device < 0) {
        fprintf(stderr, "mmq_runtime_init: invalid device %d\n", device);
        return -1;
    }
    ...
    if (info.device_count == 0) {
        fprintf(stderr, "mmq_runtime_init: no CUDA devices found\n");
        return -1;
    }
    if (device >= info.device_count) {
        fprintf(stderr, "mmq_runtime_init: device %d out of range (have %d)\n",
    ...
    if (getenv("DS4_CUDA_MMQ_Q81_PERSISTENT") && !g_q81_scratch_ptr) {
        ...
        if (err != cudaSuccess) {
            fprintf(stderr, "mmq_runtime_init: cudaMalloc(q81_scratch %zu B) failed: %s; "
            ...
            fprintf(stderr, "mmq_runtime_init: persistent Q8_1 scratch enabled (%zu B at %p)\n",
...
extern "C" int mmq_runtime_should_use(int type_x, int64_t ne11, int64_t n_experts) {
...
__global__ static void mmq_runtime_sanitize_f32_kernel(float *p, uint64_t n) {
...
static void mmq_runtime_sanitize_f32(float *p, uint64_t n, cudaStream_t stream) {
    if (!p || n == 0) return;
    mmq_runtime_sanitize_f32_kernel<<<(unsigned)((n + 255u) / 256u), 256, 0, stream>>>(p, n);
}
...
bool mmq_runtime_k_tile_supported(const char *tag, int K, int cc) {
...
int mmq_runtime_dense_impl(
    ...
    const int dev = ggml_cuda_get_device();
    const int cc  = ggml_cuda_info().devices[dev].cc;
    if (!mmq_runtime_k_tile_supported<type>(tag, K, cc)) return -1;
    ...
    if (void *scratch = mmq_runtime_aligned_q81_scratch(
    ...
    mmq_runtime_sanitize_f32(out_f32, (uint64_t)M * (uint64_t)N, stream);
    return 0;
}

} // anonymous namespace

extern "C" int mmq_runtime_q8_0_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return mmq_runtime_dense_impl<GGML_TYPE_Q8_0>("mmq_runtime_q8_0_dense", W, X, out, M, N, K, stream);
}
...
extern "C" int mmq_runtime_q8_0_quantize_ref(
...
extern "C" int mmq_runtime_q8_0_dense_preq(
        const void * W, const void * Y_q8_mmq, size_t y_bytes, float * out,
        int M, int N, int K, cudaStream_t stream) {
    const char *tag = "mmq_runtime_q8_0_dense_preq";
    if (!W || !Y_q8_mmq || !out) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
    ...
    mmq_runtime_sanitize_f32(out, (uint64_t)M * (uint64_t)N, stream);
    return 0;
}

// Dense Q8_0 D2R entry: same activation quantize + scratch treatment as
// mmq_runtime_dense_impl (incl. the S1.1a zero for the never-written tail), then
...
// through an isfinite guard.  Caller (cuda/runtime.cu) resolves W_aligned and
// gates on shape (M%128, K%1024, K<=4096) + n_tok.
extern "C" int mmq_runtime_q8_0_dense_d2r(
        const void * W_aligned, const float * X_f32, float * out_f32,
        int M, int N, int K, cudaStream_t stream) {
    const char *tag = "mmq_runtime_q8_0_dense_d2r";
    ...
    const int dev = ggml_cuda_get_device();
    const int cc  = ggml_cuda_info().devices[dev].cc;
    if (!mmq_runtime_q8_0_dense_d2r_available(cc)) {
        return -1;
    }
    ggml_backend_cuda_context * ctx = get_ctx_for_device(dev);
    if (!ctx) {
    ...
    return mmq_runtime_q8_0_dense_d2r_launch(W_aligned, src1_q8_1, out_f32,
                                         M, N, K, stream);
}
 ...
 * mmq_runtime_q8_0_dense_d2r minus the activation quantize; the caller's
...
extern "C" int mmq_runtime_q8_0_dense_d2r_preq(
    ...
    cudaMemsetAsync((char *)Y_q8_mmq + data_bytes, 0, slack_bytes, stream);
    return mmq_runtime_q8_0_dense_d2r_launch(W_aligned, Y_q8_mmq, out_f32,
                                         M, N, K, stream);
}

extern "C" int mmq_runtime_q2_K_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return mmq_runtime_dense_impl<GGML_TYPE_Q2_K>("mmq_runtime_q2_K_dense", W, X, out, M, N, K, stream);
}

extern "C" int mmq_runtime_iq2_xxs_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return mmq_runtime_dense_impl<GGML_TYPE_IQ2_XXS>("mmq_runtime_iq2_xxs_dense", W, X, out, M, N, K, stream);
}

extern "C" int mmq_runtime_q4_K_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return mmq_runtime_dense_impl<GGML_TYPE_Q4_K>("mmq_runtime_q4_K_dense", W, X, out, M, N, K, stream);
}

extern "C" int mmq_runtime_mxfp4_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return mmq_runtime_dense_impl<GGML_TYPE_MXFP4>("mmq_runtime_mxfp4_dense", W, X, out, M, N, K, stream);
}
...
int mmq_runtime_moe_impl(
    ...
    ds4_pool_set_stream(stream);  /* task #22: pool ops must be stream-ordered with the kernels (see mmq_runtime_dense_impl) */
    ...
    if (type == GGML_TYPE_Q2_K && x_soa != nullptr && d2r_enabled() &&
        ...
        if (d2r_avail_cc != cc) {
            d2r_avail_cc = cc;
            d2r_avail = mmq_runtime_q2_K_moe_d2r_available(cc) ? 1 : 0;
        }
        if (d2r_avail) {
            const size_t d2r_work_bytes =
                mmq_runtime_q2_K_moe_d2r_scratch_bytes(ne_get_rows, n_experts);
            if (d2r_work_bytes != 0) {
                ggml_cuda_pool_alloc<char> d2r_work(ctx->pool(), d2r_work_bytes);
                const int d2r_rc = mmq_runtime_q2_K_moe_d2r_launch(
    ...
    if (sanitize_out) {
        mmq_runtime_sanitize_f32(out_f32, (uint64_t)M * (uint64_t)ne_get_rows, stream);
    }
    return 0;
}

struct mmq_runtime_fused_down {
    const void  * W;
    const char  * W_soa;
    int64_t       soa_blocks;
     ...
     * fused_direct_soa doc in mmq_runtime.h); NULL = quantize internally. */
    const void  * input_q8_ext;
    size_t        input_q8_ext_bytes;
};

static bool mmq_runtime_take_scratch(
...
static bool mmq_runtime_scratch_overlaps(
...
// header comment on mmq_runtime_iq2_xxs_moe_pair for motivation.  Internal
// structure mirrors mmq_runtime_moe_impl above; the only differences are the
...
int mmq_runtime_moe_pair_impl(
         ...
         * shape, so one block count); see mmq_runtime_moe_impl. */
        const char    * xa_soa     = NULL,
        const char    * xb_soa     = NULL,
        int64_t         soa_blocks = 0,
        /* ds4 (P3): see mmq_runtime_moe_impl. */
        bool            sanitize_out = true,
        const mmq_runtime_fused_down *fused_down = nullptr) {
    ...
    const bool nvtx_prefill = profile_fused_prefill &&
                              fused_down != nullptr &&
                              n_tokens >= 1024 &&
                              mmq_runtime_nvtx_requested();
    mmq_runtime_nvtx_scope fused_scope(
            "ds4/prefill/moe/mmq_fused",
            mmq_runtime_nvtx_payload((uint32_t)n_tokens, (uint32_t)n_expert_used),
            nvtx_prefill);
    ...
    if (direct_gateup_q8) {
        ...
        const size_t gateup_work_bytes =
            mmq_runtime_iq2_xxs_moe_d2r_fused_scratch_bytes(
        ...
        if (!mmq_runtime_iq2_xxs_moe_d2r_available(cc) ||
            !mmq_runtime_q2_K_moe_d2r_available(cc)) {
            return -96;
        }
    ...
    // Same cap guard as mmq_runtime_moe_impl (see comment there): past the smem
    ...
    {
        mmq_runtime_nvtx_scope stage(
                "ds4/prefill/moe/expert_map",
                mmq_runtime_nvtx_payload((uint32_t)n_tokens, (uint32_t)n_experts),
                nvtx_prefill);
        // Task #22 root-cause fix (same as mmq_runtime_moe_impl): zero the id maps
    ...
    if (!input_q8_ext) {
        mmq_runtime_nvtx_scope stage(
                ...
                mmq_runtime_nvtx_payload((uint32_t)quant_rows, (uint32_t)K),
                nvtx_prefill);
        ybuf_memset(src1_q8_1, nbytes_src1_q8_1, stream);
        if (use_native_fp4) {
            quantize_mmq_fp4_cuda(
    ...
    if (direct_gateup_q8) {
        ...
        }
        {
            mmq_runtime_nvtx_scope stage(
                    ...
                    mmq_runtime_nvtx_payload((uint32_t)ne_get_rows, (uint32_t)M),
                    nvtx_prefill);
            const int d2r_rc = mmq_runtime_iq2_xxs_moe_d2r_fused_launch(
        ...
        }
        {
            mmq_runtime_nvtx_scope stage(
                    ...
                    mmq_runtime_nvtx_payload((uint32_t)ne_get_rows,
                                         (uint32_t)fused_down->out_dim),
                    nvtx_prefill);
            const int down_rc = mmq_runtime_q2_K_moe_d2r_launch(
                    fused_down->W_soa,
    ...
    if (type == GGML_TYPE_IQ2_XXS && xa_soa != nullptr && xb_soa != nullptr &&
        ...
        if (d2r_iq2_avail_cc != cc) {
            d2r_iq2_avail_cc = cc;
            d2r_iq2_avail = mmq_runtime_iq2_xxs_moe_d2r_available(cc) ? 1 : 0;
        }
        if (d2r_iq2_avail) {
            const size_t d2r_work_bytes =
                mmq_runtime_iq2_xxs_moe_d2r_pair_scratch_bytes(ne_get_rows, n_experts);
            if (d2r_work_bytes != 0) {
                ggml_cuda_pool_alloc<char> d2r_work(ctx->pool(), d2r_work_bytes);
                mmq_runtime_nvtx_scope stage(
                        ...
                        mmq_runtime_nvtx_payload((uint32_t)ne_get_rows, (uint32_t)M),
                        nvtx_prefill);
                const int d2r_rc = mmq_runtime_iq2_xxs_moe_d2r_pair_launch(
            ...
            if (work_bytes != 0u) {
                ...
                mmq_runtime_nvtx_scope stage(
                        ...
                        mmq_runtime_nvtx_payload((uint32_t)ne_get_rows,
                                             (uint32_t)fused_down->out_dim),
                        nvtx_prefill);
                down_done = mmq_runtime_q2_K_moe_d2r_launch(
                        fused_down->W_soa,
                        fused_down->soa_blocks,
                        down_q8_1.get(),
                        ids_dst,
    ...
    if (sanitize_out) {
        mmq_runtime_sanitize_f32(out_a, (uint64_t)M * (uint64_t)ne_get_rows, stream);
        mmq_runtime_sanitize_f32(out_b, (uint64_t)M * (uint64_t)ne_get_rows, stream);
    }
    return 0;
}
...
extern "C" int mmq_runtime_q8_0_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_impl<GGML_TYPE_Q8_0>("mmq_runtime_q8_0_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q2_K_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_impl<GGML_TYPE_Q2_K>("mmq_runtime_q2_K_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_iq2_xxs_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_impl<GGML_TYPE_IQ2_XXS>("mmq_runtime_iq2_xxs_moe", W, X, ids, out, M, K,
                                               n_tokens, n_experts, n_expert_used, stream);
}
...
extern "C" int mmq_runtime_q2_K_moe_soa(
        const void * W_soa, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    if (M <= 0 || M % 2 != 0 || K <= 0 || K % 256 != 0 || n_experts <= 0) {
        fprintf(stderr, "mmq_runtime_q2_K_moe_soa: bad shape M=%d K=%d nexp=%d\n", M, K, n_experts);
        return -1;
    }
    ...
    return mmq_runtime_moe_impl<GGML_TYPE_Q2_K>("mmq_runtime_q2_K_moe_soa", W_soa, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream,
                                            (const char *)W_soa, npair,
                                            /*sanitize_out=*/false);
}

extern "C" int mmq_runtime_q4_K_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_impl<GGML_TYPE_Q4_K>("mmq_runtime_q4_K_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_mxfp4_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_impl<GGML_TYPE_MXFP4>("mmq_runtime_mxfp4_moe", W, X, ids, out, M, K,
                                             n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_iq2_xxs_moe_pair(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_impl<GGML_TYPE_IQ2_XXS>(
        "mmq_runtime_iq2_xxs_moe_pair", W_a, W_b, X, ids, out_a, out_b,
 ...
 * mmq_runtime_q2_K_moe_soa. */
extern "C" int mmq_runtime_iq2_xxs_moe_pair_soa(
        const void * Wa_soa, const void * Wb_soa,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    if (M <= 0 || K <= 0 || K % 256 != 0 || n_experts <= 0) {
        fprintf(stderr, "mmq_runtime_iq2_xxs_moe_pair_soa: bad shape M=%d K=%d nexp=%d\n", M, K, n_experts);
        return -1;
    }
    const int64_t nblk = (int64_t)n_experts * (int64_t)M * (int64_t)(K/256);
    /* sanitize_out=false: see mmq_runtime_q2_K_moe_soa. */
    return mmq_runtime_moe_pair_impl<GGML_TYPE_IQ2_XXS>(
        "mmq_runtime_iq2_xxs_moe_pair_soa", Wa_soa, Wb_soa, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream,
...
extern "C" int mmq_runtime_iq2_xxs_q2_K_moe_fused_soa(
    ...
    const mmq_runtime_fused_down fused_down = {
        W_down,
        (const char *)W_down,
        q2_pairs,
        router_weights,
    ...
    return mmq_runtime_moe_pair_impl<GGML_TYPE_IQ2_XXS, true>(
        "mmq_runtime_iq2_xxs_q2_K_moe_fused_soa",
...
extern "C" int mmq_runtime_iq2_xxs_q2_K_moe_fused_direct_scratch_sizes(
        int expert_mid_dim,
        int expert_in_dim,
        int n_tokens,
        int n_experts,
    ...
    const size_t down_work = mmq_runtime_q2_K_moe_d2r_scratch_bytes(
        (int64_t)rows, n_experts);
...
extern "C" int mmq_runtime_iq2_xxs_q2_K_moe_fused_direct_soa(
        const void * W_gate, const void * W_up, const void * W_down,
        const float * X, const int32_t * ids, const float * router_weights,
    ...
    return mmq_runtime_moe_pair_impl<GGML_TYPE_IQ2_XXS, true>(
        "mmq_runtime_iq2_xxs_q2_K_moe_fused_direct_soa",
...
extern "C" int mmq_runtime_q4_K_moe_pair(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_impl<GGML_TYPE_Q4_K>(
        "mmq_runtime_q4_K_moe_pair", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_mxfp4_moe_pair(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_impl<GGML_TYPE_MXFP4>(
        "mmq_runtime_mxfp4_moe_pair", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}
...
int mmq_runtime_moe_vec_impl(
    ...
    // fall back to the pool path.  See mmq_runtime_init for setup.
    ggml_cuda_pool_alloc<char> src1_q8_1_pool;
    ...
    mmq_runtime_sanitize_f32(out_f32, (uint64_t)M * (uint64_t)n_tokens * (uint64_t)n_expert_used, stream);
    return 0;
}
...
//
// Layout contract (see mmq_runtime.h): W_aligned = [__half dq[nblk]][pad to 64B]
...
// mmq_runtime_moe_gate_up_mid_q8_1_qwarp32_kernel) -> mid directly.  Replaces
...
template <ggml_type type>
int mmq_runtime_moe_pair_raw_vec_impl(
        const char    * tag,
        const void    * W_a,
        const void    * W_b,
        const float   * X_f32,
    ...
    mmq_runtime_sanitize_f32(out_a, out_count, stream);
    mmq_runtime_sanitize_f32(out_b, out_count, stream);
    return 0;
}

template <ggml_type type>
int mmq_runtime_moe_pair_vec_impl(
        const char    * tag,
        const void    * W_a,
        const void    * W_b,
        const float   * X_f32,
    ...
    // for Step 8 / CUDA Graph compatibility.  See mmq_runtime_moe_vec_impl.
    ds4_pool_set_stream(stream);
    ...
    mmq_runtime_sanitize_f32(out_silu, (uint64_t)M * (uint64_t)n_expert_used, stream);
    return 0;
}

template <ggml_type type>
int mmq_runtime_dense_vec_impl(
        const char  * tag,
        const void  * W,
        const float * X_f32,
        float       * out_f32,
...
template <ggml_type type> struct mmq_runtime_vdr_mmvq_value;
template <> struct mmq_runtime_vdr_mmvq_value<GGML_TYPE_IQ2_XXS> { static constexpr int value = VDR_IQ2_XXS_Q8_1_MMVQ; };
template <> struct mmq_runtime_vdr_mmvq_value<GGML_TYPE_Q2_K>    { static constexpr int value = VDR_Q2_K_Q8_1_MMVQ; };
template <> struct mmq_runtime_vdr_mmvq_value<GGML_TYPE_Q4_K>    { static constexpr int value = VDR_Q4_K_Q8_1_MMVQ; };
template <> struct mmq_runtime_vdr_mmvq_value<GGML_TYPE_MXFP4>   { static constexpr int value = VDR_MXFP4_Q8_1_MMVQ; };

template <ggml_type type>
static __device__ __forceinline__ float mmq_runtime_vec_dot_q8_1(
...
static __device__ __forceinline__ float mmq_runtime_half_warp_sum_f32(float v) {
...
template <ggml_type type>
static __global__ void mmq_runtime_moe_down_sum6_q8_1_qwarp32_kernel(
        const void       * __restrict__ W,
        const block_q8_1 * __restrict__ X_q8,
    ...
    constexpr int vdr = mmq_runtime_vdr_mmvq_value<type>::value;
...
#pragma unroll
        for (uint32_t slot = 0; slot < top_k; ++slot) {
            ...
            for (uint32_t b = kbx0; !invalid_id && b < blocks_per_row_x; b += blocks_per_iter) {
                acc += mmq_runtime_vec_dot_q8_1<type>(
                    W, xq + (uint64_t)b * q8_per_k, kbx_base + (int)b, kqs);
            }
            acc = mmq_runtime_half_warp_sum_f32(acc);
            if (lane == 0) {
                if (!isfinite(acc)) acc = 0.0f;
                total += acc;
            }
...
template <ggml_type type>
static __global__ void mmq_runtime_moe_gate_up_mid_q8_1_qwarp32_kernel(
...
#pragma unroll
    for (uint32_t rr = 0; rr < 4u; ++rr) {
        ...
        for (uint32_t b = kbx0; !invalid_id && b < blocks_per_row_x; b += blocks_per_iter) {
            ...
            gate += mmq_runtime_vec_dot_q8_1<type>(W_gate, xb, kbx, kqs);
            up   += mmq_runtime_vec_dot_q8_1<type>(W_up,   xb, kbx, kqs);
        }
        gate = mmq_runtime_half_warp_sum_f32(gate);
        up   = mmq_runtime_half_warp_sum_f32(up);
        if (lane == 0) {
            if (!isfinite(gate)) gate = 0.0f;
            if (!isfinite(up)) up = 0.0f;
            if (clamp > 1.0e-6f) {
...
static __global__ void mmq_runtime_moe_down_sum6_vec_kernel(
        const void       * __restrict__ W,
        const block_q8_1 * __restrict__ X_q8,
        const int32_t    * __restrict__ ids,
        float            * __restrict__ out,
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            tmp[i] += mmq_runtime_vec_dot_q8_1<type>(
...
template <ggml_type type>
int mmq_runtime_moe_down_sum6_vec_impl(
        const char    * tag,
        const void    * W,
        const float   * X_f32,
        const int32_t * ids,
    ...
    mmq_runtime_moe_down_sum6_q8_1_qwarp32_kernel<type><<<block_nums, block_dims, 0, stream>>>(
...
template <ggml_type type, int c_rows_per_block>
static __global__ void mmq_runtime_moe_gate_up_mid_vec_kernel(
        const void       * __restrict__ W_gate,
        const void       * __restrict__ W_up,
        const block_q8_1 * __restrict__ X_q8,
        const int32_t    * __restrict__ ids,
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            ...
            gate[i] += mmq_runtime_vec_dot_q8_1<type>(W_gate, &y[kby], row_kbx, kqs);
            up[i]   += mmq_runtime_vec_dot_q8_1<type>(W_up,   &y[kby], row_kbx, kqs);
        }
    }
...
template <ggml_type type, int c_rows_per_block>
static __global__ void mmq_runtime_moe_gate_up_mid_vec_by_slot_kernel(
        const void       * __restrict__ W_gate,
        const void       * __restrict__ W_up,
        const block_q8_1 * __restrict__ X_q8,
        const int32_t    * __restrict__ ids,
...
template <ggml_type type>
int mmq_runtime_moe_gate_up_mid_vec_impl(
        const char    * tag,
        const void    * W_gate,
        const void    * W_up,
        const float   * X_f32,
    ...
    char *src1_q8_1_ptr = mmq_runtime_folded_q81(X_f32, K, n_tokens, ne10_padded);
    cudaError_t err;
    if (!src1_q8_1_ptr) {
    if (g_q81_scratch_enabled && g_q81_scratch_ptr && g_q81_scratch_bytes >= nbytes_q8_1) {
        src1_q8_1_ptr = (char *)g_q81_scratch_ptr;
    ...
    const dim3 block_dims(256);
    mmq_runtime_moe_gate_up_mid_q8_1_qwarp32_kernel<type><<<block_nums, block_dims, 0, stream>>>(
...
__global__ static void mmq_runtime_q4_K_dense_pair_vec_kernel(
...
#pragma unroll
        for (uint32_t rr = 0; rr < 4u; rr++) {
            ...
            if (row < M) {
                ...
                acc0[rr] += mmq_runtime_vec_dot_q8_1<type>(W0, xb, kbx, kqs);
                acc1[rr] += mmq_runtime_vec_dot_q8_1<type>(W1, xb, kbx, kqs);
            }
        }
    }
#pragma unroll
    for (uint32_t rr = 0; rr < 4u; rr++) {
        acc0[rr] = mmq_runtime_half_warp_sum_f32(acc0[rr]);
        acc1[rr] = mmq_runtime_half_warp_sum_f32(acc1[rr]);
    }
    if (lane == 0u) {
#pragma unroll
        for (uint32_t rr = 0; rr < 4u; rr++) {
...
static int mmq_runtime_q4_K_dense_pair_vec_impl(
    ...
    char *x8 = mmq_runtime_folded_q81(X, K, 1, padded);
    if (!x8) {
        if (void *scratch = mmq_runtime_aligned_q81_scratch(dev, qbytes)) {
            x8 = (char *)scratch;
    ...
    mmq_runtime_q4_K_dense_pair_vec_kernel
...
extern "C" int mmq_runtime_q8_0_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_vec_impl<GGML_TYPE_Q8_0>(
        "mmq_runtime_q8_0_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q2_K_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_vec_impl<GGML_TYPE_Q2_K>(
        "mmq_runtime_q2_K_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_iq2_xxs_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_vec_impl<GGML_TYPE_IQ2_XXS>(
        "mmq_runtime_iq2_xxs_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q4_K_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_vec_impl<GGML_TYPE_Q4_K>(
        "mmq_runtime_q4_K_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_mxfp4_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_vec_impl<GGML_TYPE_MXFP4>(
        "mmq_runtime_mxfp4_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

// M1-Inc2b: exact inverse of the weight-server repack
// (repack_iq2_xxs_aligned_kernel, original weight-server tool): aligned-SoA
...
extern "C" int mmq_runtime_iq2_xxs_aligned_derepack(
        const void * W_aligned, void * raw_out,
        int M, int K, int n_experts, cudaStream_t stream) {
    const char *tag = "mmq_runtime_iq2_xxs_aligned_derepack";
...
extern "C" uint64_t mmq_runtime_q8_0_aligned_bytes(int M, int K) {
...
extern "C" int mmq_runtime_q8_0_aligned_dense_vec_pair(
    ...
    const char *tag = "mmq_runtime_q8_0_aligned_dense_vec_pair";
    ...
    const size_t qbytes =
        (size_t)padded * sizeof(block_q8_1) / QK8_1;
    ggml_cuda_pool_alloc<char> q8_pool;
    char *x8 = mmq_runtime_folded_q81(X_f32, K, 1, padded);
    if (!x8) {
        if (g_q81_scratch_enabled && g_q81_scratch_ptr &&
            g_q81_scratch_bytes >= qbytes) {
            x8 = (char *)g_q81_scratch_ptr;
...
extern "C" int mmq_runtime_q8_0_aligned_dense_vec(
        const void * W_aligned, const float * X_f32, float * out_f32,
        int M, int N, int K, cudaStream_t stream) {
    const char *tag = "mmq_runtime_q8_0_aligned_dense_vec";
    if (!W_aligned || !X_f32 || !out_f32) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
    ...
    char *x8 = N == 1 ? mmq_runtime_folded_q81(X_f32, K, 1, ne10_padded) : NULL;
    cudaError_t err;
    if (!x8) {
    if (g_q81_scratch_enabled && g_q81_scratch_ptr && g_q81_scratch_bytes >= nbytes_q8_1) {
        x8 = (char *)g_q81_scratch_ptr;
...
// --repack-q2k-aligned, DERIVED_Q2_K_ALIGNED_MOE, and mmq_runtime.h):
//
...
// original weight-server tool): pair-SoA -> raw block_q2_K byte stream. One
...
extern "C" uint64_t mmq_runtime_q2_k_aligned_bytes(int M, int K, int n_experts) {
...
extern "C" int mmq_runtime_q2_K_aligned_moe_vec(
    ...
    const char *tag = "mmq_runtime_q2_K_aligned_moe_vec";
    ...
    /* Quantize verbatim from mmq_runtime_moe_vec_impl<GGML_TYPE_Q2_K> so the q8_1
    ...
    ggml_cuda_pool_alloc<char> src1_q8_1_pool;
    char *src1_q8_1_ptr = nullptr;
    if (void *scratch = mmq_runtime_aligned_q81_scratch(dev, nbytes_q8_1)) {
    ...
    mmq_runtime_sanitize_f32(out_f32, (uint64_t)M * (uint64_t)n_tokens, stream);
    return 0;
}

extern "C" int mmq_runtime_q2_K_aligned_derepack(
        const void * W_aligned, void * raw_out,
        int M, int K, int n_experts, cudaStream_t stream) {
    const char *tag = "mmq_runtime_q2_K_aligned_derepack";
    if (!W_aligned || !raw_out) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
...
extern "C" uint64_t mmq_runtime_iq2_xxs_aligned_bytes(int M, int K, int n_experts) {
...
static char *iq2_aligned_quantize_xn(
    ...
    char *folded = mmq_runtime_folded_q81(X_f32, K, n_tokens, ne10_padded);
    if (folded) {
...
extern "C" int mmq_runtime_iq2_xxs_aligned_moe_gate_up_mid_vec(
        const void * W_gate_aligned, const void * W_up_aligned,
        const float * X_f32, const int32_t * ids, const float * weights,
        float * mid_f32,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    const char *tag = "mmq_runtime_iq2_xxs_aligned_moe_gate_up_mid_vec";
...
extern "C" int mmq_runtime_iq2_xxs_aligned_moe_vec(
        const void * W_aligned, const float * X_f32, const int32_t * ids, float * out_f32,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    const char *tag = "mmq_runtime_iq2_xxs_aligned_moe_vec";
    if (!W_aligned || !X_f32 || !ids || !out_f32) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
    ...
    // Quantize X into canonical Q8_1, exactly as mmq_runtime_moe_vec_impl does, so
    ...
    mmq_runtime_sanitize_f32(out_f32, (uint64_t)n_tokens * (uint64_t)M * (uint64_t)n_expert_used, stream);
    return 0;
}

extern "C" int mmq_runtime_q2_K_moe_down_sum6_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_down_sum6_vec_impl<GGML_TYPE_Q2_K>(
        "mmq_runtime_q2_K_moe_down_sum6_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q4_K_moe_down_sum6_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_down_sum6_vec_impl<GGML_TYPE_Q4_K>(
        "mmq_runtime_q4_K_moe_down_sum6_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_mxfp4_moe_down_sum6_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_down_sum6_vec_impl<GGML_TYPE_MXFP4>(
        "mmq_runtime_mxfp4_moe_down_sum6_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_iq2_xxs_moe_gate_up_mid_vec(
        const void * W_gate, const void * W_up,
        const float * X, const int32_t * ids, const float * weights, float * mid,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    return mmq_runtime_moe_gate_up_mid_vec_impl<GGML_TYPE_IQ2_XXS>(
        "mmq_runtime_iq2_xxs_moe_gate_up_mid_vec", W_gate, W_up, X, ids, weights, mid,
        M, K, n_tokens, n_experts, n_expert_used, clamp, stream);
}

extern "C" int mmq_runtime_q4_K_moe_gate_up_mid_vec(
        const void * W_gate, const void * W_up,
        const float * X, const int32_t * ids, const float * weights, float * mid,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    return mmq_runtime_moe_gate_up_mid_vec_impl<GGML_TYPE_Q4_K>(
        "mmq_runtime_q4_K_moe_gate_up_mid_vec", W_gate, W_up, X, ids, weights, mid,
        M, K, n_tokens, n_experts, n_expert_used, clamp, stream);
}

extern "C" int mmq_runtime_mxfp4_moe_gate_up_mid_vec(
        const void * W_gate, const void * W_up,
        const float * X, const int32_t * ids, const float * weights, float * mid,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    return mmq_runtime_moe_gate_up_mid_vec_impl<GGML_TYPE_MXFP4>(
        "mmq_runtime_mxfp4_moe_gate_up_mid_vec", W_gate, W_up, X, ids, weights, mid,
        M, K, n_tokens, n_experts, n_expert_used, clamp, stream);
}

extern "C" int mmq_runtime_iq2_xxs_moe_pair_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_silu,
        int M, int K, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_vec_impl<GGML_TYPE_IQ2_XXS>(
        "mmq_runtime_iq2_xxs_moe_pair_vec", W_a, W_b, X, ids, out_silu,
        M, K, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q4_K_moe_pair_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_silu,
        int M, int K, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_vec_impl<GGML_TYPE_Q4_K>(
        "mmq_runtime_q4_K_moe_pair_vec", W_a, W_b, X, ids, out_silu,
        M, K, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_iq2_xxs_moe_pair_raw_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_raw_vec_impl<GGML_TYPE_IQ2_XXS>(
        "mmq_runtime_iq2_xxs_moe_pair_raw_vec", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q4_K_moe_pair_raw_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return mmq_runtime_moe_pair_raw_vec_impl<GGML_TYPE_Q4_K>(
        "mmq_runtime_q4_K_moe_pair_raw_vec", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int mmq_runtime_q8_0_dense_vec(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return mmq_runtime_dense_vec_impl<GGML_TYPE_Q8_0>(
        "mmq_runtime_q8_0_dense_vec", W, X, out, M, N, K, stream);
}

extern "C" int mmq_runtime_q4_K_dense_pair_vec(
        const void *W0, const void *W1, const float *X,
        float *out0, float *out1, int M, int K, cudaStream_t stream) {
    return mmq_runtime_q4_K_dense_pair_vec_impl(
