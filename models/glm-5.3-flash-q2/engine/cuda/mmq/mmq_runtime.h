...
// mmq_runtime.h - public C ABI for ds4's quantized matmul kernels.
//
// All functions are extern "C" so engine.c / cuda/runtime.cu can call them
...
// mul_mat_q_case<Q8_0> but is not yet wired into cuda/runtime.cu.
...
int mmq_runtime_init(int device);
void mmq_runtime_set_aligned_q81_scratch(void *ptr, size_t bytes);

// Query whether mmq_runtime is willing to handle a given matmul. Returns
...
int mmq_runtime_should_use(int type_x, int64_t ne11, int64_t n_experts);
...
// v0.5 flat-pool p5a: same contract as mmq_runtime_q8_0_dense but the
...
// on any non-zero.
int mmq_runtime_q8_0_dense_preq(
    const void  * W_q8_0,
    const void  * Y_q8_mmq,
    size_t        y_bytes,
    float       * out_f32,
...
int mmq_runtime_q8_0_quantize_ref(
    const float * X,
    void        * y,
    size_t        y_bytes,
    int           N,
...
// --repack-q8-aligned).  Same in/out contract as mmq_runtime_q8_0_dense but W is
...
int mmq_runtime_q8_0_dense_d2r(
...
int mmq_runtime_q8_0_dense_d2r_preq(
    const void  * W_aligned,
    const void  * Y_q8_mmq,
    size_t        y_bytes,
    float       * out_f32,
...
// ds4 (P4 Inc3): same contract as mmq_runtime_q2_K_moe but W_soa is the aligned
// row-pair-SoA artifact (weight server --repack-q2k-aligned, layout in
// mmq_runtime_q2_k_aligned_bytes' comment) instead of the raw block stream.  The
...
// as a single mmq_runtime_<type>_moe call. Saves one launch of
// quantize_mmq_q8_1_cuda and one ggml_cuda_launch_mm_ids_helper per MoE
// block. See mmq_runtime.cu / routed_moe_launch for the wiring.
//
// Returns 0 on success; on error neither output is guaranteed valid.

int mmq_runtime_iq2_xxs_moe_pair(
    const void    * W_a,
    const void    * W_b,
    const float   * X_f32,
    const int32_t * ids,
...
// ds4 (P4 Inc3): same contract as mmq_runtime_iq2_xxs_moe_pair but over the
// aligned-SoA artifacts (weight server --repack-iq2-aligned); see
// mmq_runtime_q2_K_moe_soa.
int mmq_runtime_iq2_xxs_moe_pair_soa(
    const void    * Wa_soa,
    const void    * Wb_soa,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_iq2_xxs_q2_K_moe_fused_soa(
...
int mmq_runtime_iq2_xxs_q2_K_moe_fused_direct_soa(
    const void    * W_gate_soa,
    const void    * W_up_soa,
    const void    * W_down_soa,
    const float   * X_f32,
...
int mmq_runtime_iq2_xxs_q2_K_moe_fused_direct_scratch_sizes(
    int     expert_mid_dim,
    int     expert_in_dim,
    int     n_tokens,
    int     n_experts,
...
int mmq_runtime_mxfp4_moe_pair(
    const void    * W_a,
    const void    * W_b,
    const float   * X_f32,
    const int32_t * ids,
...
// mmq_runtime_<type>_moe entries above, but route through llama.cpp's mmvq
...
int mmq_runtime_iq2_xxs_moe_vec(
    const void    * W,
    const float   * X_f32,
    const int32_t * ids,
    float         * out_f32,
...
// mmq_runtime_iq2_xxs_aligned_bytes to size or validate an artifact.
//
// Semantics and output layout are identical to mmq_runtime_iq2_xxs_moe_vec at
// n_tokens == 1 (the only supported width; other widths return non-zero so
// the caller can fall back).
uint64_t mmq_runtime_iq2_xxs_aligned_bytes(int M, int K, int n_experts);
...
int mmq_runtime_iq2_xxs_aligned_derepack(
    const void    * W_aligned,
    void          * raw_out,
    int             M,
    int             K,
...
//   [ uint2 qs2[npair*16] ]
//
// keyed to the kernel's rows_per_block == 2 (see mmq_runtime.cu for the exact
// field packing).  Use mmq_runtime_q2_k_aligned_bytes to size or validate an
// artifact.  Unsupported shapes return non-zero so the caller can fall back.
uint64_t mmq_runtime_q2_k_aligned_bytes(int M, int K, int n_experts);

int mmq_runtime_q2_K_aligned_moe_vec(
    const void    * W_aligned,
    const float   * X_f32,
    const int32_t * ids,
    float         * out_f32,
...
// mmq_runtime_iq2_xxs_aligned_derepack).
int mmq_runtime_q2_K_aligned_derepack(
    const void    * W_aligned,
    void          * raw_out,
    int             M,
    int             K,
...
// return non-zero so the caller can fall back to mmq_runtime_q8_0_dense_vec.
uint64_t mmq_runtime_q8_0_aligned_bytes(int M, int K);

int mmq_runtime_q8_0_aligned_dense_vec(
    const void  * W_aligned,
    const float * X_f32,
    float       * out_f32,
    int           M,
...
int mmq_runtime_q8_0_aligned_dense_vec_pair(
...
int mmq_runtime_iq2_xxs_aligned_moe_vec(
    const void    * W_aligned,
    const float   * X_f32,
    const int32_t * ids,
    float         * out_f32,
...
// epilogue (identical semantics to mmq_runtime_moe_gate_up_mid_q8_1_qwarp32) and
// writes mid[slot * M + row] directly.
int mmq_runtime_iq2_xxs_aligned_moe_pair_vec(
...
int mmq_runtime_iq2_xxs_aligned_moe_gate_up_mid_vec(
    const void    * W_gate_aligned,
    const void    * W_up_aligned,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_q2_K_moe_down_sum6_vec(
    const void    * W,
    const float   * X_f32,
    const int32_t * ids,
    float         * out_f32,
...
int mmq_runtime_q4_K_moe_down_sum6_vec(
    const void    * W,
    const float   * X_f32,
    const int32_t * ids,
    float         * out_f32,
...
int mmq_runtime_mxfp4_moe_down_sum6_vec(
    const void    * W,
    const float   * X_f32,
    const int32_t * ids,
    float         * out_f32,
...
//
// Clamp semantics match cuda/runtime.cu: gate is capped above, up is clamped to
...
int mmq_runtime_iq2_xxs_moe_gate_up_mid_vec(
    const void    * W_gate,
    const void    * W_up,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_q4_K_moe_gate_up_mid_vec(
    const void    * W_gate,
    const void    * W_up,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_mxfp4_moe_gate_up_mid_vec(
    const void    * W_gate,
    const void    * W_up,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_iq2_xxs_moe_pair_vec(
    const void    * W_a,
    const void    * W_b,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_q4_K_moe_pair_vec(
    const void    * W_a,
    const void    * W_b,
    const float   * X_f32,
    const int32_t * ids,
...
// mmq_runtime_*_moe_vec calls.
//
// Layout and constraints match mmq_runtime_*_moe_vec:
//   out_[token * n_expert_used + slot, row]
//   K must be a multiple of 256.

int mmq_runtime_iq2_xxs_moe_pair_raw_vec(
    const void    * W_a,
    const void    * W_b,
    const float   * X_f32,
    const int32_t * ids,
...
int mmq_runtime_q4_K_moe_pair_raw_vec(
    const void    * W_a,
    const void    * W_b,
    const float   * X_f32,
    const int32_t * ids,
...
// mmq_runtime_q8_0_dense but routed through mmvq for batch counts that
// favour the vec path (n_tokens <= 8 on Blackwell).
//
// Returns 0 on success, non-zero on validation or launch failure.

int mmq_runtime_q8_0_dense_vec(
    const void  * W_q8_0,
    const float * X_f32,
    float       * out_f32,
    int           M,
...
int mmq_runtime_q4_K_dense_pair_vec(
    const void  * W0_q4_K,
    const void  * W1_q4_K,
    const float * X_f32,
    float       * out0_f32,
