/* repack: shared aligned-artifact layout library.  See repack.h for
 * the API surface and the original weight-server tool for the source of
...
bool repack_map_file(const char *log_prefix, const char *path, repack_file &m) {
...
void repack_unmap_file(repack_file &m) {
...
static bool read_u32(const repack_file &m, uint64_t &pos, uint32_t &out) {
...
static bool read_u64(const repack_file &m, uint64_t &pos, uint64_t &out) {
...
static bool skip_bytes(const repack_file &m, uint64_t &pos, uint64_t n) {
    if (pos > m.size || n > m.size - pos) return false;
    pos += n;
    return true;
}

static bool read_string(const repack_file &m, uint64_t &pos, std::string &out) {
    uint64_t len = 0;
...
static bool skip_metadata_value(const repack_file &m, uint64_t &pos, uint32_t type);

static bool skip_array(const repack_file &m, uint64_t &pos) {
...
bool repack_collect_catalog(const char *log_prefix,
                                const repack_file &m,
                                std::vector<repack_span> *spans,
                                std::vector<repack_tensor> *records) {
    uint64_t pos = 0;
    ...
    std::vector<repack_tensor> tensors;
...
bool repack_pread_full(int fd, void *buf, uint64_t bytes, uint64_t offset) {
    uint64_t done = 0;
    while (done < bytes) {
...
bool repack_read_stage(const repack_file &m, void *stage, uint64_t stage_bytes,
    ...
    *payload = (const char *)stage;
    ...
    if (m.direct_fd >= 0 && m.direct_align > 1 && m.size != 0) {
        ...
        if (aligned_off <= m.size && read_size <= stage_bytes && read_size <= m.size - aligned_off) {
            ...
            errno = 0;
            if (repack_pread_full(m.direct_fd, stage, read_size, aligned_off)) {
    ...
    (void)stage_bytes;
#endif
    return repack_pread_full(m.fd, stage, bytes, file_off);
}
 ...
 * engine's lazy dequant_q8_0_to_f16_kernel (cuda/runtime.cu) verbatim -- the
...
bool repack_iq2_candidate(const repack_tensor &t) {
 ...
 * adjacent rows). */
bool repack_q2k_candidate(const repack_tensor &t) {
...
bool repack_q8_candidate(const repack_tensor &t) {
...
bool repack_q8_f16_candidate(const repack_tensor &t) {
    if (t.type != 8u || t.ndim != 2u) return false; /* GGML_TYPE_Q8_0 */
    if (t.dims[0] == 0 || t.dims[1] == 0) return false;
...
void repack_set_threads(int n) {
    g_repack_threads = n;
}

void repack_set_hash(bool on) {
...
uint64_t repack_fnv1a(const unsigned char *p, uint64_t n, uint64_t h) {
...
static bool run_repack_jobs(const char *log_prefix, const char *what, const repack_file &m,
    ...
    auto worker = [&]() {
        ...
        if (err != cudaSuccess) {
            ...
            while (!fail.load(std::memory_order_relaxed)) {
                ...
                if (hash) {
                    const repack_artifact &r = jobs[i].art;
...
static bool repack_read_upload(const char *log_prefix, const repack_file &m, const char *what,
                               const repack_tensor &t,
                               unsigned char *stage, uint64_t stage_bytes, unsigned char *scratch,
                               uint64_t done, uint64_t nb, cudaStream_t stream) {
    const char *payload = nullptr;
    if (!repack_read_stage(m, stage, stage_bytes, t.off + done, nb, &payload)) {
...
static bool repack_alloc_artifact(const repack_build_args &a, const char *what,
                                  repack_artifact *art) {
...
static void repack_free_artifact(const repack_build_args &a, repack_artifact *art) {
    if (!art->dev) return;
 ...
 * mmq_runtime_q8_0_aligned_dense_vec (cuda/mmq/mmq_runtime.h).  ADDITIVE: the raw
 * spans stay served, so only the n_tokens==1 decode GEMV dispatches on the
 * artifact; every other consumer reads raw as before. */
bool repack_build_q8_aligned(const repack_build_args &a,
                                 std::vector<repack_artifact> &out,
                                 uint64_t *repacked_bytes_out) {
    if (repacked_bytes_out) *repacked_bytes_out = 0;
    repack_file m;
    if (!repack_map_file(a.log_prefix, a.path, m)) return false;
    ...
    bool ok = true;
    for (const repack_tensor &t : *a.records) {
        if (!repack_q8_candidate(t)) continue;
...
bool repack_build_q8_f16(const repack_build_args &a,
                             std::vector<repack_artifact> &out,
                             uint64_t *repacked_bytes_out) {
    if (repacked_bytes_out) *repacked_bytes_out = 0;
    repack_file m;
    ...
    bool ok = true;
    for (const repack_tensor &t : *a.records) {
        if (!repack_q8_f16_candidate(t)) continue;
        const uint64_t nblk = t.bytes / 34u;
        const uint64_t expect_blk = (t.dims[0] / 32u) * t.dims[1];
        if (nblk != expect_blk || t.off > m.size || t.bytes > m.size - t.off) {
            fprintf(stderr,
 ...
 * mmq_runtime_iq2_xxs_aligned_moe_vec (cuda/mmq/mmq_runtime.h).  These REPLACE the
...
bool repack_build_iq2_aligned(const repack_build_args &a,
                                  std::vector<repack_artifact> &out,
                                  uint64_t *repacked_bytes_out) {
    if (repacked_bytes_out) *repacked_bytes_out = 0;
    repack_file m;
    ...
    bool ok = true;
    for (const repack_tensor &t : *a.records) {
        if (!repack_iq2_candidate(t)) continue;
 ...
 * Layout contract shared with mmq_runtime_q2_K_aligned_moe_vec
 * (cuda/mmq/mmq_runtime.h).  Chunks are whole row pairs so the repack kernel's
 * global pair index math holds across chunk boundaries. */
bool repack_build_q2k_aligned(const repack_build_args &a,
                                  std::vector<repack_artifact> &out,
                                  uint64_t *repacked_bytes_out) {
    if (repacked_bytes_out) *repacked_bytes_out = 0;
    repack_file m;
    ...
    bool ok = true;
    for (const repack_tensor &t : *a.records) {
        if (!repack_q2k_candidate(t)) continue;
