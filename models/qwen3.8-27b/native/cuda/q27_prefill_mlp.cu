...
q27_prefill_mlp_status VerifyNvfp4(const char* operation,
                                   q27_verify_nvfp4_status status) {
  return Kernel(operation, status.message);
}
...
bool BuildVerifyT8Layout(q27_prefill_mlp_layout* output,
                         q27_verify_nvfp4_shape* gate_up,
                         q27_verify_nvfp4_shape* down) {
  *gate_up = {sizeof(*gate_up), Q27_VERIFY_NVFP4_ABI_VERSION};
  *down = {sizeof(*down), Q27_VERIFY_NVFP4_ABI_VERSION};
  q27_verify_nvfp4_status status =
      q27_verify_nvfp4_query(Q27_VERIFY_NVFP4_GATE_UP, gate_up);
  if (status.code != Q27_VERIFY_NVFP4_OK) return false;
  status = q27_verify_nvfp4_query(Q27_VERIFY_NVFP4_DOWN, down);
  if (status.code != Q27_VERIFY_NVFP4_OK) return false;
  ...
  output->activated_offset = cursor;
  const uint64_t activated_bytes =
      static_cast<uint64_t>(Q27_VERIFY_NVFP4_ROWS) * kIntermediate * 2;
  if (!Add(&cursor, activated_bytes)) return false;
  output->packed_input_offset = cursor;
  if (!Add(&cursor,
           std::max(gate_up->packed_input_bytes, down->packed_input_bytes)))
  ...
  if (!Add(&cursor,
           std::max(gate_up->input_scale_bytes, down->input_scale_bytes)))
    return false;
  output->tokens = Q27_VERIFY_NVFP4_ROWS;
  output->reserved = 0;
  output->scratch_bytes = cursor;
  output->scratch_alignment = kAlignment;
  output->workspace_bytes =
...
extern "C" q27_prefill_mlp_status q27_prefill_mlp_query_verify_t8(
    q27_prefill_mlp_layout* output) {
  if (output == nullptr || output->struct_size < sizeof(*output) ||
      output->abi_version != Q27_PREFILL_MLP_ABI_VERSION) {
    return Invalid("invalid Q27 T8 verify MLP layout query");
  }
  q27_verify_nvfp4_shape gate_up{};
  q27_verify_nvfp4_shape down{};
  if (!BuildVerifyT8Layout(output, &gate_up, &down)) {
    return Kernel("query Q27 T8 verify MLP",
                  "unsupported fixed M=8 NVFP4 shape");
  }
  return Ok();
}

static q27_prefill_mlp_status ForwardVerifyT8Impl(
    const q27_prefill_mlp_args* args) {
  if (args == nullptr || args->struct_size < sizeof(*args) ||
      args->abi_version != Q27_PREFILL_MLP_ABI_VERSION ||
      args->tokens != Q27_VERIFY_NVFP4_ROWS) {
    return Invalid("invalid Q27 T8 verify MLP ABI header or token count");
  }
  ...
  q27_verify_nvfp4_shape gate_up{};
  q27_verify_nvfp4_shape down{};
  if (!BuildVerifyT8Layout(&layout, &gate_up, &down)) {
    return Kernel("prepare Q27 T8 verify MLP",
                  "unsupported fixed M=8 NVFP4 shape");
  }
  if (!Aligned(args->input_bf16, 16) ||
    ...
    return Invalid(
        "Q27 T8 verify MLP pointer, alignment, or byte-size mismatch");
  }
  ...
  q27_verify_nvfp4_project_args projection{};
  projection.struct_size = sizeof(projection);
  projection.abi_version = Q27_VERIFY_NVFP4_ABI_VERSION;
  projection.projection = Q27_VERIFY_NVFP4_GATE_UP;
  ...
  q27_verify_nvfp4_status status = q27_verify_nvfp4_project(&projection);
  if (status.code != Q27_VERIFY_NVFP4_OK)
    return VerifyNvfp4("Q27 T8 verify merged gate/up", status);

  const uint64_t activated_elements =
      static_cast<uint64_t>(Q27_VERIFY_NVFP4_ROWS) * kIntermediate;
  ...
  if (error != cudaSuccess)
    return CudaError("Q27 T8 verify SiLU multiply", error);

  projection.projection = Q27_VERIFY_NVFP4_DOWN;
  ...
  status = q27_verify_nvfp4_project(&projection);
  return status.code == Q27_VERIFY_NVFP4_OK
             ? Ok()
             : VerifyNvfp4("Q27 T8 verify down projection", status);
}
...
extern "C" q27_prefill_mlp_status q27_prefill_mlp_forward_verify_t8(
    const q27_prefill_mlp_args* args) {
  return ForwardVerifyT8Impl(args);
}
