...
//
// Fixed M=8 Qwen3.8-27B target NVFP4 projection. Arithmetic is reused from
// FlashInfer 906181e3 and its pinned CUTLASS b46b16d0 subtree; this file only
// freezes model shapes, validates raw buffers, and calls those launchers.

#include "q27_verify_nvfp4.h"
...
#if defined(__GNUC__)
#define Q27_VERIFY_WEAK __attribute__((weak))
#else
#define Q27_VERIFY_WEAK
#endif

/*
 * These exact AOT symbols are generated from an M-symbolic CuTe kernel. Weak
 * declarations let an intentionally incomplete development link return
 * UNIMPLEMENTED instead of pretending that an M=1-only quantizer is valid.
 * The supported Spark build links both pinned objects strongly.
 */
extern "C" int
__tvm_ffi_nvfp4_quantize_swizzled_bfloat16_k5120_sf0_pdl0(
    void* handle, const TVMFFIAny* args, int32_t num_args,
    TVMFFIAny* result) Q27_VERIFY_WEAK;
extern "C" int
...
constexpr uint32_t kRows = Q27_VERIFY_NVFP4_ROWS;
constexpr uint32_t kScaleRows = Q27_VERIFY_NVFP4_SCALE_ROWS;
constexpr uint32_t kScaleGroup = Q27_VERIFY_NVFP4_SCALE_GROUP;
constexpr uint32_t kHidden = Q27_VERIFY_NVFP4_HIDDEN_SIZE;
constexpr uint32_t kIntermediate = Q27_VERIFY_NVFP4_INTERMEDIATE_SIZE;
constexpr uint32_t kMergedIntermediate = 2 * kIntermediate;
constexpr uint32_t kQuantizeBlocks = kScaleRows;
...
q27_verify_nvfp4_status Ok() { return {Q27_VERIFY_NVFP4_OK, "ok"}; }

q27_verify_nvfp4_status Invalid(const char* message) {
  return {Q27_VERIFY_NVFP4_INVALID_ARGUMENT, message};
}

q27_verify_nvfp4_status Unsupported(const char* message) {
  return {Q27_VERIFY_NVFP4_UNSUPPORTED_SHAPE, message};
}

q27_verify_nvfp4_status Unimplemented(const char* message) {
  return {Q27_VERIFY_NVFP4_UNIMPLEMENTED, message};
}

q27_verify_nvfp4_status Error(q27_verify_nvfp4_status_code code,
...
bool ResolveShape(uint32_t projection, uint32_t* n, uint32_t* k) {
  switch (projection) {
    case Q27_VERIFY_NVFP4_GATE:
    case Q27_VERIFY_NVFP4_UP:
      *n = kIntermediate;
      *k = kHidden;
      return true;
    case Q27_VERIFY_NVFP4_GATE_UP:
      *n = kMergedIntermediate;
      *k = kHidden;
      return true;
    case Q27_VERIFY_NVFP4_DOWN:
      *n = kHidden;
      *k = kIntermediate;
      return true;
    default:
...
CutlassGemmConfig VerifyConfig(bool stream_k) {
  return CutlassGemmConfig(
      ...
      /*swap_ab=*/true, /*use_stream_k=*/stream_k);
}
...
size_t RunGemm(void* output, const void* input, const void* weight,
  ...
  const bool stream_k = n == static_cast<int>(kIntermediate) ||
                        n == static_cast<int>(kMergedIntermediate);
  if (stream_k) {
    return flashinfer::gemm::genericFp4GemmKernelLauncherStreamK<
        ...
        true>(output, input, weight, input_scales, weight_scales, alpha,
              /*m=*/kRows, n, k, /*l=*/1, VerifyConfig(true), workspace,
  ...
  return flashinfer::gemm::genericFp4GemmKernelLauncher<
      ...
      cute::Int<1>, cute::Int<1>, cute::Int<1>, flashinfer::gemm::_1SM,
      true>(output, input, weight, input_scales, weight_scales, alpha,
            /*m=*/kRows, n, k, /*l=*/1, VerifyConfig(false), workspace,
            workspace_bytes, stream, nullptr);
}

q27_verify_nvfp4_shape ExpectedShape(uint32_t n, uint32_t k,
                                     uint64_t workspace) {
  q27_verify_nvfp4_shape shape = {};
  shape.struct_size = sizeof(shape);
  shape.abi_version = Q27_VERIFY_NVFP4_ABI_VERSION;
  shape.m = kRows;
  shape.n = n;
  shape.k = k;
  shape.padded_scale_rows = kScaleRows;
  shape.scale_group = kScaleGroup;
  shape.input_bf16_bytes =
      static_cast<uint64_t>(kRows) * k * sizeof(__nv_bfloat16);
  shape.packed_input_bytes = static_cast<uint64_t>(kRows) * k / 2;
  shape.input_scale_bytes =
  ...
  shape.output_bf16_bytes =
      static_cast<uint64_t>(kRows) * n * sizeof(__nv_bfloat16);
...
q27_verify_nvfp4_status Quantize(
    const q27_verify_nvfp4_quantize_args& args, uint32_t k, int device_id) {
  ...
  int64_t input_shape[2] = {kRows, columns};
  int64_t input_strides[2] = {columns, 1};
  int64_t output_shape[2] = {kRows, packed_columns};
  int64_t output_strides[2] = {packed_columns, 1};
  int64_t scale_shape[1] = {kScaleRows * scale_columns};
  ...
  TVMFFIAny call_args[7] = {
      ...
      IntegerArgument(kRows), IntegerArgument(kScaleRows),
      // Each pinned K uses rows_per_block=1. Overlaunching all padded rows is
      // legal and keeps this fixed-address launch independent of host queries.
      IntegerArgument(kQuantizeBlocks), TensorArgument(&global_scale),
  };
  TVMFFIAny result = {};
  result.type_index = kTVMFFINone;
  const int status = quantizer(nullptr, call_args, 7, &result);
  if (status != 0)
    return {Q27_VERIFY_NVFP4_INTERNAL_ERROR,
            "pinned symbolic-M FlashInfer CuTe quantizer call failed"};
  return Ok();
}

q27_verify_nvfp4_status Query(uint32_t projection,
                              q27_verify_nvfp4_shape* output) {
  if (output == nullptr || output->struct_size < sizeof(*output) ||
      output->abi_version != Q27_VERIFY_NVFP4_ABI_VERSION)
    return Invalid("Q27 verify NVFP4 shape output ABI mismatch");
  uint32_t n = 0;
  uint32_t k = 0;
  if (!ResolveShape(projection, &n, &k))
    return Unsupported("projection is not a pinned Q27 M=8 NVFP4 shape");
  size_t workspace = 0;
  try {
    ...
    return Error(Q27_VERIFY_NVFP4_INTERNAL_ERROR,
                 "FlashInfer M=8 workspace query failed: ",
                 exception.what());
  } catch (...) {
    return {Q27_VERIFY_NVFP4_INTERNAL_ERROR,
            "FlashInfer M=8 workspace query raised an unknown error"};
  }
  *output = ExpectedShape(n, k, workspace);
  return Ok();
}

bool ValidQuantizeBuffers(const q27_verify_nvfp4_quantize_args* args,
                          const q27_verify_nvfp4_shape& shape) {
...
bool ValidGemmBuffers(const q27_verify_nvfp4_gemm_args* args,
                      const q27_verify_nvfp4_shape& shape) {
...
extern "C" q27_verify_nvfp4_status q27_verify_nvfp4_query(
    uint32_t projection, q27_verify_nvfp4_shape* output) {
  return Query(projection, output);
}

extern "C" q27_verify_nvfp4_status q27_verify_nvfp4_quantize(
    const q27_verify_nvfp4_quantize_args* args) {
  if (args == nullptr || args->struct_size < sizeof(*args) ||
      args->abi_version != Q27_VERIFY_NVFP4_ABI_VERSION)
    return Invalid("Q27 verify NVFP4 quantize ABI mismatch");
  q27_verify_nvfp4_shape shape = {sizeof(shape),
                                   Q27_VERIFY_NVFP4_ABI_VERSION};
  q27_verify_nvfp4_status status = Query(args->projection, &shape);
  if (status.code != Q27_VERIFY_NVFP4_OK) return status;
  if (!ValidQuantizeBuffers(args, shape))
    return Invalid(
        "Q27 verify NVFP4 quantize pointer, alignment, or exact byte size mismatch");

  int device_id = 0;
  cudaError_t error = cudaGetDevice(&device_id);
  if (error != cudaSuccess)
    return Error(Q27_VERIFY_NVFP4_CUDA_ERROR, "cannot resolve CUDA device: ",
                 cudaGetErrorString(error));
  StreamScope stream_scope(device_id, args->cuda_stream);
  if (stream_scope.status() != 0)
    return {Q27_VERIFY_NVFP4_INTERNAL_ERROR,
            "cannot set TVM-FFI stream for M=8 NVFP4 quantization"};
  status = Quantize(*args, shape.k, device_id);
  if (status.code != Q27_VERIFY_NVFP4_OK) return status;
  error = cudaPeekAtLastError();
  return error == cudaSuccess
             ? Ok()
             : Error(Q27_VERIFY_NVFP4_CUDA_ERROR,
                     "Q27 M=8 NVFP4 quantize launch failed: ",
                     cudaGetErrorString(error));
}

extern "C" q27_verify_nvfp4_status q27_verify_nvfp4_gemm(
    const q27_verify_nvfp4_gemm_args* args) {
  if (args == nullptr || args->struct_size < sizeof(*args) ||
      args->abi_version != Q27_VERIFY_NVFP4_ABI_VERSION)
    return Invalid("Q27 verify NVFP4 GEMM ABI mismatch");
  q27_verify_nvfp4_shape shape = {sizeof(shape),
                                   Q27_VERIFY_NVFP4_ABI_VERSION};
  q27_verify_nvfp4_status status = Query(args->projection, &shape);
  if (status.code != Q27_VERIFY_NVFP4_OK) return status;
  if (!ValidGemmBuffers(args, shape))
    return Invalid(
        "Q27 verify NVFP4 GEMM pointer, alignment, or exact byte size mismatch");
  ...
  try {
    ...
    return Error(Q27_VERIFY_NVFP4_INTERNAL_ERROR,
                 "FlashInfer Q27 M=8 NVFP4 launch failed: ",
                 exception.what());
  } catch (...) {
    return {Q27_VERIFY_NVFP4_INTERNAL_ERROR,
            "FlashInfer Q27 M=8 NVFP4 launch raised an unknown error"};
  }
  const cudaError_t error = cudaPeekAtLastError();
  return error == cudaSuccess
             ? Ok()
             : Error(Q27_VERIFY_NVFP4_CUDA_ERROR,
                     "Q27 M=8 NVFP4 GEMM launch failed: ",
                     cudaGetErrorString(error));
}

extern "C" q27_verify_nvfp4_status q27_verify_nvfp4_project(
    const q27_verify_nvfp4_project_args* args) {
  if (args == nullptr || args->struct_size < sizeof(*args) ||
      args->abi_version != Q27_VERIFY_NVFP4_ABI_VERSION)
    return Invalid("Q27 verify NVFP4 project ABI mismatch");

  q27_verify_nvfp4_quantize_args quantize = {};
  quantize.struct_size = sizeof(quantize);
  quantize.abi_version = Q27_VERIFY_NVFP4_ABI_VERSION;
  ...
  q27_verify_nvfp4_gemm_args gemm = {};
  gemm.struct_size = sizeof(gemm);
  gemm.abi_version = Q27_VERIFY_NVFP4_ABI_VERSION;
  ...
  /* Reject the entire combined call before quantization has side effects. */
  q27_verify_nvfp4_shape shape = {sizeof(shape),
                                   Q27_VERIFY_NVFP4_ABI_VERSION};
  q27_verify_nvfp4_status status = Query(args->projection, &shape);
  if (status.code != Q27_VERIFY_NVFP4_OK) return status;
  if (!ValidQuantizeBuffers(&quantize, shape) ||
    ...
    return Invalid(
        "Q27 verify NVFP4 project pointer, alignment, or exact byte size "
        "mismatch");
  }
  status = q27_verify_nvfp4_quantize(&quantize);
  if (status.code != Q27_VERIFY_NVFP4_OK) return status;
  return q27_verify_nvfp4_gemm(&gemm);
}
