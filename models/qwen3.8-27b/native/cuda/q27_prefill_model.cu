// Fixed-M128, allocation-free Qwen3.8-27B target prefill composition.
...
constexpr uint64_t kLogitsBytes = 248320ULL * 4;
...
q27_prefill_model_status BuildLayout(const q27_prefill_model_config* config,
  ...
  if (!ConfigValid(config) || output == nullptr ||
  ...
  q27_gdn_prefill_layer_status gs = q27_gdn_prefill_layer_query(&gdn);
  ...
  const uint64_t attention_shared =
      Align(Q27_PREFILL_ATTENTION_LAYER_SCRATCH_BYTES) +
      Align(config->fp8_workspace_bytes) +
      Q27_PREFILL_ATTENTION_WORKSPACE_BYTES(config->cache_capacity);
  const uint64_t mlp_shared = Align(mlp.scratch_bytes) + mlp.workspace_bytes;
  output->shared_bytes = Align(std::max(
      gdn.scratch_bytes, std::max(attention_shared, mlp_shared)));
  output->scratch_bytes = output->shared_offset + output->shared_bytes;
  output->scratch_alignment = kAlign;
  output->gdn_state_bytes_per_layer = gdn.recurrent_state_bytes;
  output->gdn_conv_bytes_per_layer = gdn.convolution_state_bytes;
  output->attention_cache_bytes_per_layer =
      2ULL * config->cache_capacity * 1024ULL;
  return Ok();
}

q27_prefill_model_status CreateFp8(uint32_t n, uint32_t k,
...
bool CommonCallValid(const q27_prefill_model_plan* plan,
  ...
  return plan != nullptr && args != nullptr &&
         ...
         args->valid_tokens >= 1 && args->valid_tokens <= 128 &&
...
extern "C" q27_prefill_model_status q27_prefill_model_query(
  ...
  return BuildLayout(config, output);
}
...
extern "C" q27_prefill_model_status q27_prefill_model_plan_create(
  ...
  *output = nullptr;
  ...
  q27_prefill_model_status status = BuildLayout(config, &layout);
  ...
  plan->config = *config;
  plan->layout = layout;
  status = CreateFp8(16384, 5120, *config, &plan->gdn_qkvz);
  if (status.code == Q27_PREFILL_MODEL_OK)
    status = CreateFp8(5120, 6144, *config, &plan->gdn_out);
  if (status.code == Q27_PREFILL_MODEL_OK) {
...
extern "C" q27_prefill_model_status q27_prefill_model_forward(
  ...
  q27_prefill_core_status core = q27_prefill_embedding(&embedding);
  ...
  q27_prefill_mlp_status ml = q27_prefill_mlp_query(128, &mlp_layout);
  ...
  for (uint32_t layer_index = 0; layer_index < 64; ++layer_index) {
    ...
    if (layer.kind == Q27_PREFILL_MODEL_GDN) {
      ...
      aa.scratch = common;
      aa.scratch_bytes = Q27_PREFILL_ATTENTION_LAYER_SCRATCH_BYTES;
      ...
      aa.attention_workspace_bytes =
          Q27_PREFILL_ATTENTION_WORKSPACE_BYTES(plan->config.cache_capacity);
    ...
    ma.input_bf16_bytes = kHiddenBytes;
    ...
    ma.output_bf16_bytes = kHiddenBytes;
  ...
  if (args->produce_output == 0) return Ok();
  ...
  GatherLastRow<<<20, 256, 0, stream>>>(
      static_cast<const __nv_bfloat16*>(arena.normalized), args->valid_tokens,
      static_cast<__nv_bfloat16*>(arena.last_hidden));
  cudaError_t cuda = cudaPeekAtLastError();
  ...
  argmax.output_token_i32 = static_cast<int32_t*>(args->output_token_i32);
  argmax.cuda_stream = args->cuda_stream;
  kernel = q27_argmax(&argmax);
  return kernel.code == Q27_KERNEL_OK ? Ok()
